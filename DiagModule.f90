...
!!
  subroutine FindEvals(electrons)
    ...
    do i=1,nkp ! Build and send Hamiltonian for each k-point
       ...
       if(iprint_DM>=3.AND.myid==0) write(io_lun,*) myid,' Done DistributeCQ_to_SC for S'
       ! Now, if this processor is involved, do the diagonalisation
       if(iprint_DM>=3.AND.myid==0) write(io_lun,*) myid,'Proc row, cols, me: ',proc_rows, proc_cols, me, i, nkp
