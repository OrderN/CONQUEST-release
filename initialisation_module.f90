...
!!
  subroutine initialise(vary_mu, fixed_potential, number_of_bands, mu, total_energy)
    ...
    if(flag_no_atomic_densities.AND.(.NOT.find_chdens)) then
       if(inode==ionode) write(*,*) 'No initial charge density specified - building from initial K'
       find_chdens = .true.
    end if
...
!!
  subroutine set_up(find_chdens, number_of_bands)
    ...
    if (inode==ionode.AND.iprint_init>1) write(*,*) 'Completed set_blocks()'
    ...
    call my_barrier()
    if (inode==ionode.AND.iprint_init>1) write(*,*) 'Completed set_domains()'
    ...
    do spec = 1, n_species
       ...
       if(inode==ionode.AND.iprint_init>2) write(*,*) 'Extent is: ',Extent(spec)
    end do
    ! Sorts out which processor owns which atoms
    call distribute_atoms(inode, ionode)
    call my_barrier
    if(inode==ionode.AND.iprint_init>1) write(*,*) 'Completed distribute_atoms()'
    ...
    if(inode==ionode.AND.iprint_init>1) write(*,*) ' rcut for BCS_parts =',rcut_BCS
    ...
    if(inode==ionode.AND.iprint_init>1) &
         write(*,*) 'Made covering set for matrix multiplications'
    ...
    if(inode==ionode.AND.iprint_init>1) write(*,*) 'Completed immi()'
    ...
    call my_barrier()
    if (inode==ionode.AND.iprint_init>1) write(*,*) 'Completed set_grid()'

    call associate_fn_on_grid
    call my_barrier()
    if (inode==ionode.AND.iprint_init>1) write(*,*) 'Completed associate_fn_on_grid()'
    ...
    call my_barrier()
    if (inode==ionode.AND.iprint_init>1) write(*,*) 'Completed fft init'
    ...
    ! +++
    call my_barrier
    if(inode==ionode.AND.iprint_init>1) write(*,*) 'Completed set_ewald()'
    ...
    if(inode==ionode.AND.iprint_init>1) write(*,*) 'Done init_pseudo '
...
!!
  subroutine initial_phis( mu, restart_file, read_phi, vary_mu, start)
    ...
    if(flag_basis_set==blips) then
       if(inode==ionode) write(*,fmt='(10x,"Using blips as basis set for support functions")')
       call set_blip_index(inode, ionode)
       call my_barrier
       if((inode == ionode) .AND. (iprint_init > 1)) write(*,*) 'initial_phis:&
            & completed set_blip_index()'

       !if((inode == ionode).and.(iprint_init >= 0)) then
       !   write(unit=*,fmt='(/" initial_phis: n_species:",i3)') n_species
       !   write(unit=*,fmt='(/" initial_phis: r_h:",f12.6)') r_h
       ...
       call my_barrier
       if((inode == ionode).AND.(iprint_init > 0)) then
          write(unit=*,fmt='(10x,"initial_phis: initial blip coeffs created")')
       end if
       ...
       if (start .or. (.NOT.read_option)) then
          ...
          !     write(*,*) 'S matrix for normalisation on Node= ',inode
          ...
          call my_barrier()
          if(inode==ionode.AND.iprint_init>1) &
               write(*,fmt='(10x,"Completed normalise_support")')
          call blip_to_support_new(inode-1, supportfns)
          call get_matrix_elements_new(inode-1,rem_bucket(1),matS,supportfns,supportfns)
          !call dump_matrix("NS",matS,inode)
       else
          if(inode==ionode.AND.iprint_init>1) &
               write(*,fmt='(10x,"Skipped normalise_support")')
       end if
    else if(flag_basis_set==PAOs) then
       call gen_rad_tables(inode,ionode)
       call gen_nlpf_supp_tbls(inode,ionode)
       call make_ang_coeffs
       if(inode==ionode) write(*,fmt='(10x,"Using PAOs as basis set for support functions")')
       !call make_pre_paos
       if((inode == ionode).and.(iprint_init >0)) then
          write(unit=*,fmt='(10x,"initial_phis: n_species:",i3)') n_species
          write(unit=*,fmt='(10x,"initial_phis: r_h:",f12.6)') r_h
       end if
...
!!
  subroutine initial_H( start, start_L, find_chdens, fixed_potential, vary_mu, number_of_bands, mu, total_energy)
    ...
    if(inode==ionode.AND.iprint_init>1) write(*,*) 'Got S'
    call my_barrier
    ...
    if (.NOT.diagon.AND.find_chdens.AND.(start .or. start_L)) then
       call initial_L( )
       call my_barrier()
       if (inode.eq.ionode.AND.iprint_init>1) write(*,*) 'Got L matrix'
       if(vary_mu) &
    ...
    if(.NOT.diagon.AND.(find_chdens.OR.restart_L)) then
       ...
       if(inode==ionode.AND.iprint_init>1) write(*,*) 'Got elect: ',electrons
    end if
    ...
    if(inode==ionode.AND.iprint_init>1) write(*,*) 'Calling Ewald'
    if(flag_old_ewald) then
       call ewald
    else
       call mikes_ewald
    ...
    if(inode==ionode.AND.iprint_init>2) write(*,*) 'Find_chdens is ',find_chdens
    ...
    if(find_chdens) then
       ...
       if(inode==ionode.AND.iprint_init>1) write(*,*) 'In initial_H, electrons: ',electrons
...
!!
  subroutine setgrid(myid,r_core_squared,r_h)
    ...
    if(myid == 0.AND.iprint_index>1) write(*,*) 'setgrid_new starts'
    if(iprint_index > 4) write(*,*) ' setgrid_new starts for myid= ',myid

    !Sets up domain
    call init_primary(domain, n_pts_in_block*maxblocks, maxblocks, .false.)
    if(iprint_index > 4) write(*,*) 'init_primary end for myid = ',myid,' par = ',n_pts_in_block, maxblocks
    ! call my_barrier()  !TMP
    call make_prim(blocks, domain, myid)
    if(iprint_index > 4) write(*,*) 'make_prim end for myid = ',myid
    ...
    !endif
    if(iprint_index > 4) write(*,*) ' rcut_max for DCS_parts and BCS_blocks = ',rcut_max, r_core, r_h

    call make_cs(myid,rcut_max, DCS_parts , parts , domain, ni_in_cell, x_atom_cell, y_atom_cell, z_atom_cell)
    if(iprint_index > 4) write(*,*) 'Node ',myid+1,' Done make_DCSparts'
    call make_cs(myid,rcut_max, BCS_blocks, blocks, bundle)
    if(iprint_index > 4) write(*,*) 'Node ',myid+1,' Done make_BCSblocks'

    !call make_iprim(DCS_parts,bundle,myid) !primary number for members
    !  write(*,*) 'Node ',myid+1,' Done make_iprim'
    call my_barrier

    call send_ncover(DCS_parts,myid+1)
    if(iprint_index > 4) write(*,*) 'Node ',myid+1,' Done send_ncover for DCS_parts'
    call my_barrier
    call send_ncover(BCS_blocks,myid+1)
    if(iprint_index > 4) write(*,*) 'Node ',myid+1,' Done send_ncover for BCS_blocks'

    if(iprint_index > 4) call check_setgrid

    if(iprint_index > 4) write(*,*) ' DCS & BCS has been prepared for myid = ',myid
    ...
    if(iprint_index > 4) write(*,*) 'Node ',myid+1,' Done set_blipgrid'
    ...
    if(iprint_index > 4) write(*,*) 'Node ',myid+1,' Done set_bucket'
  ...
  subroutine check_setgrid
    ...
    if(iprint_index > 4) then
       ...
       do nnd=1,numprocs
          if(myid == nnd-1) then
             write(*,*)
             write(*,*) ' Node ',nnd,' CHECK bundle '
             write(*,*) ' n_prim, groups_on_node = ', bundle%n_prim,bundle%groups_on_node
             write(*,101) bundle%nx_origin,bundle%ny_origin,bundle%nz_origin
             write(*,102) bundle%nw_primx ,bundle%nw_primy ,bundle%nw_primz
             write(*,103) bundle%nleftx   ,bundle%nlefty   ,bundle%nleftz  
...
105          format(3x,' Right  Up   TOP  ',3f15.6)
       ...
       do nnd=1,numprocs
          if(myid == nnd-1) then
             write(*,*)
             write(*,*) ' Node ',nnd,' CHECK domain '
             write(*,*) ' n_prim, groups_on_node = ', domain%n_prim,domain%groups_on_node
             write(*,101) domain%nx_origin,domain%ny_origin,domain%nz_origin
             write(*,102) domain%nw_primx ,domain%nw_primy ,domain%nw_primz
             write(*,103) domain%nleftx   ,domain%nlefty   ,domain%nleftz 
             ...
             !104 format(3x,' Left  Down Bottom',3f15.6)
             !105 format(3x,' Right  Up   TOP  ',3f15.6)
             !write(*,*) ' @@ mx_nbonn for node ',nnd,' >= ',domain%groups_on_node
       ...
       do nnd2=1,numprocs
          if(myid ==nnd2-1 ) then
             write(*,*) 
             write(*,*) ' Node ', nnd2, ' CHECK BCS_parts'
             write(*,101) BCS_parts%nx_origin, BCS_parts%ny_origin, BCS_parts%nz_origin
             write(*,102) BCS_parts%ncoverx  , BCS_parts%ncovery  , BCS_parts%ncoverz
             write(*,103) BCS_parts%nspanlx  , BCS_parts%nspanly  , BCS_parts%nspanlz
             ...
             write(*,105) xx,yy,zz
             do nnd=1,numprocs
                write(*,*) ' for Node = ',nnd,' BCSparts_ncover_remote ',&
       ...
       do nnd2=1,numprocs
          if(myid ==nnd2-1 ) then
             write(*,*)
             write(*,*) ' Node ', nnd2, ' CHECK DCS_parts'
             write(*,101) DCS_parts%nx_origin, DCS_parts%ny_origin, DCS_parts%nz_origin
             write(*,102) DCS_parts%ncoverx  , DCS_parts%ncovery  , DCS_parts%ncoverz
             write(*,103) DCS_parts%nspanlx  , DCS_parts%nspanly  , DCS_parts%nspanlz
             ...
             enddo
             !write(*,*) ' @@ mx_pcover_DCS for node ',nnd2,' >= ',DCS_parts%ng_cover
             ...
             enddo
             !write(*,*) ' @@ mx_icover_DCS for node ',nnd2,' >= ',&
       ...
       do nnd2=1,numprocs
          if(myid ==nnd2-1 ) then
             write(*,*)
             write(*,*) ' Node ', nnd2, ' CHECK BCS_blocks'
             write(*,101) BCS_blocks%nx_origin, BCS_blocks%ny_origin, BCS_blocks%nz_origin
             write(*,102) BCS_blocks%ncoverx  , BCS_blocks%ncovery  , BCS_blocks%ncoverz
             write(*,103) BCS_blocks%nspanlx  , BCS_blocks%nspanly  , BCS_blocks%nspanlz
             ...
             enddo
             write(*,*) ' @@ mx_bcover for node ',nnd2,' >= ',BCS_blocks%ng_cover
