...
!!$
    ...
    if (.not. flag_MDold .and. restart_T) then
       ...
       call my_barrier()
       call Matrix_CommRebuild(Info,Trange,T_trans,matT(1),nfile,symm)
       if (nspin_SF==2) then
          call grab_matrix2('T2',inode,nfile,Info)
          call my_barrier()
          call Matrix_CommRebuild(Info,Trange,T_trans,matT(2),nfile,symm)
       endif
    endif
