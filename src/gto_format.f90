...
module gto_format_new
  ...
  type gto_prim
     integer :: ngto
     ! Gaussian primitive functions info
     real(double), allocatable :: a(:)   ! orbital exponent 
     real(double), allocatable :: d(:)   ! contraction coefficient
     real(double), allocatable :: c(:)   ! origin 
  end type gto_prim
  ...
  type angmom_gto
     ...
     integer, pointer  :: norm(:)
     !integer, pointer  :: kind  ! kind of orbital: 0 = standard, 1 = polarisation
     !real(double)      :: occ   ! occupation      
     character(len=8), pointer :: nt(:) !  (s,px,py...)
  end type angmom_gto
