...
filename_gto_write='NaCQ.gto_TZTP_3G' 
# file to read in order to build an initial guess (not mandatory)
filename_gto_read ='NaCQ.gto_TZTP_3G_guess' 
...
for i in range(norb):
    ...
    center = False
    bounds = ([-6,-5,-4,-3,-2,-1],[6,4,5,6,7,10],) #Bounds for a,d and c
    param, nG = gto_fit_orb( x, y, orb[i].nG, orb[i].guess, orb[i].n, orb[i].lname, orb[i].z, i, 
                            center=center, maxfev=200000, method='dogbox', bounds = bounds)
