...
filename_gto_write='AlCQ.gto_DZP_3G' 
# file to read in order to build an initial guess (not mandatory)
#filename_gto_read ='AlCQ.gto_DZP_3G_guess' 
...
# orb[1].guess = array([ 0.1,-1.5,-1,1,1,1,1,1,1])
...
for i in range(norb):
    ...
    param, nG = gto_fit_orb( x, y, orb[i].nG, orb[i].guess, orb[i].n, orb[i].lname, orb[i].z, i, 
                            center=center, maxfev=2000000, method='dogbox', bounds = bounds)
