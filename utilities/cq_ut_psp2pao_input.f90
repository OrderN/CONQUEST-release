...
! $Id$
! ------------------------------------------------------------------------------
! Module cq_ut_psp2pao_input
! ------------------------------------------------------------------------------
! $Log$
! ------------------------------------------------------------------------------

!!****h* Conquest/cq_ut_psp2pao_input
!!  NAME
!!   cq_ut_psp2pao_input
!!  PURPOSE
!!   This implements some of the functionality of FDF but operating on an character array
!!   This is just an adaptation from the main code, to avoid conflict of types
!!  AUTHOR
!!   D.R.Bowler
!!  CREATION DATE
!!   2008/08/20
...
!!   2008/09/03 ast
!!     Adaptation from main code (name of the modules, error output)
!!  SOURCE
!!  
module cq_ut_psp2pao_input
...
!!****f* cq_ut_psp2pao_input/load_input *
!!
!!  NAME 
!!   load_input
!!  USAGE
...
!!  
  subroutine load_input
    ...
    do while(.NOT.done)
       ...
       if(good_line) then
          if(l>input_lines) write(*,*) "Input reading error !",l,input_lines
          input_array(l) = line
          l = l+1
       end if
    end do
    ...
    open(unit=fdf_out,file='fdf.out')
    i =  fdf_integer('fdf-debug',0)
    if(i>0) then
       fdf_debug = .true.
       call io_assign(fdf_log)
       open(unit=fdf_log,file='fdf.log')
    else
       fdf_debug = .false.
    end if
    return
...
!
    ...
    if (ntokens==1) then
       write(fdf_out,*) 'FDF_INTEGER: No value for ', label
       write(*,*) "Input problem: integer not found"
    endif
...
!
    ...
    if (ntokens==1) then
       write(fdf_out,*) 'FDF_DOUBLE: No value for ', label
       write(*,*) "Input problem: double not found"
    endif
...
!     is not found in the fdf file.
  ...
  logical function fdf_boolean(label,default)
    ...
    if (ntokens .eq. 1) then
       fdf_boolean = .true.
       write(fdf_out,'(a,5x,l10,5x,a)') label, fdf_boolean, '# Label by itself'
       return
    endif
    ...
    if (leqi(valstr,'yes') .or. &
       ...
       write(*,*) "FDF_BOOLEAN: Unexpected fdf logical value "//label//" "//valstr
    endif
    return
  ...
  logical function fdf_block(label)
    ...
    if (.not. leqi(token1,'%block')) then
       write(fdf_log,*) 'FDF_BLOCK: Not a block:',label
!        Return instead of stopping
    ...
    do while(.NOT.done)
       if(fdf_getline()) then
          ...
          write(*,*) "Block fails to end: "//label
       end if
    end do
...
!
  ...
  logical function fdf_getline()
    ...
    if(current_line<=maxlines) then
       ...
       if (fdf_debug) write(fdf_log,'(a,a76)') '> ', line
       call fdf_parse
       return
    else
       fdf_getline = .false.
...
!
  ...
  subroutine io_assign(lun)
    ...
    write(*,*) "Error in io_assign: no free luns between ",lun_min,lun_max
...
end module cq_ut_psp2pao_input
