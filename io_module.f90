...
!!  USES
!!   GenComms, basic_types, common, datatypes, global_module, group_module, matrix_module, species_module
!!  AUTHOR
!!    D.R.Bowler
!!  CREATION DATE   
...
!!   24/06/2002 dave
!!    Changed read_atoms to use fdf io_assign routines (not raw open/close)
!!   14:00, 04/02/2003 drb 
!!    Added dump/grab routines for charge, matrices, blips, local and non-local pseudos for 
!!    force checking
!!   16:43, 2003/04/15 dave
!!    Added read_atomic_coordinates and changed read_mult
!!   16:55, 2003/06/09 dave
!!    Further changes based on TM's work: now use atom_coord instead of various x_atom etc variables
!!    Changes also to mapping and numbering
...
!!
  subroutine read_atomic_positions(filename)
    ...
    use global_module, ONLY: x_atom_cell, y_atom_cell, z_atom_cell, &
         ...
         rcellx, rcelly, rcellz, id_glob, iprint_init, &
         id_glob_inv, atom_coord, species_glob, flag_move_atom, area_init
...
first:    do
             ...
             case ('ATOM  ','HETATM')
                if (lgt( pdb_line(17:17),' ')) then 
                   ! File with alternate locations and no General.PdbAltLoc specified?
                   !
                   ! This check fails if General.PdbAltLoc is in the input file but
                   ! has no value. In this case, ALL entries with altloc will be
                   ! ignored. User beware. (Of course we can always sell it as a feature.)
                   if (leqi( pdb_altloc,' ') ) then
                      call io_close(lun)
                      call cq_abort('PDB file with alternate locations, specify General.PdbAltLoc')
                   endif
                   ! Count the alternate location entry?
                   if ( leqi( pdb_line(17:17), pdb_altloc)) ni_in_cell = ni_in_cell + 1
                else
...
second:   do
                ...
                if (lgt( pdb_line(17:17),' ')) then
                   ...
                   if ( leqi( pdb_line(17:17), pdb_altloc)) then
                      ...
                      if (num_move_atom_real - (real(num_move_atom)) > very_small ) &
                           call cq_abort('The constraints on atom(s) have not been set. Check the pdb file.')
                      if (num_move_atom .gt. 7) & 
                           call cq_abort('Out of range value for flag_move_atom. Check the pdb file.')
                      if (num_move_atom .ge. 4) then
                         flag_move_atom(3,i) = .true.
                         num_move_atom = num_move_atom - 4
                      end if
                      if (num_move_atom .ge. 2) then
             ...
             case ('CRYST1')
                read (pdb_line,'(6x,3f9.3,3f7.2,15x)') r_super_x, r_super_y, r_super_z, angle
...
4               format(/10x,'The simulation box has the following dimensions',/, &
    ...
    if(inode/=ionode) allocate(flag_move_atom(3,ni_in_cell),atom_coord(3,ni_in_cell),species_glob(ni_in_cell),STAT=stat)
    if(stat/=0) call cq_abort("Failure to allocate coordinates: ",ni_in_cell)
    allocate(id_glob(ni_in_cell), id_glob_inv(ni_in_cell),x_atom_cell(ni_in_cell),&
         y_atom_cell(ni_in_cell),z_atom_cell(ni_in_cell),species(ni_in_cell),STAT=stat)
...
!!
  subroutine read_partitions(parts,part_file)
    ...
    ! Module usage
    use datatypes
    use global_module, ONLY: numprocs, iprint_init, ni_in_cell, id_glob, id_glob_inv
    use maxima_module, ONLY: maxpartsproc, maxatomspart, maxatomsproc, maxpartscell
    use basic_types
    use construct_module, ONLY: init_group
    ...
    if(np_in_cell<=0) call cq_abort('read_partitions: no partitions ! ',np_in_cell)
    ! Read and check number of processors
    read(lun,*) nnode
    if((nnode<1).or.(nnode>numprocs)) call cq_abort('read_partitions: nodes wrong ',nnode,numprocs)
    ...
    do nnd=1,nnode
       ...
       if(ntmp1>0) then
          ...
          if(ntmp2 + ntmp1 -1>np_in_cell) call cq_abort('read_partitions: too many parts in cell ',&
                  ntmp1 + ntmp2-1, np_in_cell)
          do np=1,ntmp1
             read(lun,*) np1,ind_part,n_cont,n_beg
             if(np1/=np) call cq_abort('read_partitions: part label error ',np1,np)
             if(n_cont>maxatomspart) maxatomspart = n_cont
             ntmp3 = ntmp3 + n_cont
             if(n_cont>0) then
                do ni=1,n_cont
                   read(lun,*) ni1,map
                   if(ni1/=ni) call cq_abort('read_partitions: atom label wrong ',ni1,ni)
                enddo
             endif
       ...
       if(parts%ng_on_node(nnd).gt.0) then
          ...
          if(parts%inode_beg(nnd)+parts%ng_on_node(nnd)-1>parts%mx_gcell) then
             call cq_abort('read_partitions: too many parts in cell ',&
                  parts%inode_beg(nnd)+parts%ng_on_node(nnd)-1, parts%mx_gcell)
          endif
          ...
          do np=1,parts%ng_on_node(nnd)
             ...
             if(n_cont>0) then
                ! Check maxima
                if(parts%icell_beg(ind_part)+n_cont-1>ni_in_cell) call cq_abort('read_partitions: too many atoms ', &
...
!!  
  subroutine create_sfc_partitions(myid, parts)
    ...
    use datatypes
    use global_module, ONLY: numprocs, iprint_init, ni_in_cell, id_glob, id_glob_inv, &
                             atom_coord, sorted_coord, global_maxatomspart, load_balance, &
                             many_processors
    use maxima_module, ONLY: maxpartsproc, maxatomspart, maxatomsproc, maxpartscell
...
!!
  subroutine dump_matrix(stub,matA,inode)
    ...
    use datatypes
    use mult_module, ONLY: return_matrix_value_pos, return_matrix_len, matrix_index, return_matrix_value
