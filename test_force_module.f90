...
module test_force_module
  ...
  use timer_stdclocks_module, ONLY: start_timer, stop_timer, &
       tmr_std_moveatoms, tmr_std_allocation
...
!!
  subroutine test_forces (fixed_potential, vary_mu, n_L_iterations, &
    ...
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, flag_perform_cDFT, &
         flag_self_consistent, ni_in_cell, flag_spin_polarisation
    use GenComms, ONLY: myid, inode, ionode
    use io_module, ONLY: dump_charge, grab_charge, dump_matrix, &
         grab_matrix, dump_blips, grab_blips, grab_locps, dump_locps, &
         grab_projs, dump_projs
    use mult_module, ONLY: matH, matK, matH_dn, matK_dn, matNL, matKE
    use energy, ONLY: hartree_energy, local_ps_energy, xc_energy, &
         nl_energy, kinetic_energy, band_energy, delta_E_hartree, &
         delta_E_xc, get_energy
    use ewald_module, ONLY: ewald_energy
    use pseudopotential_common, ONLY: core_correction, pseudopotential
    use H_matrix_module, ONLY: get_H_matrix
    use density_module, ONLY: set_density, density, density_up, density_dn
    ...
    real(double) :: expected_reduction, mu
    ...
    if (inode == ionode) write (io_lun,fmt='(2x,"******************"/,&
         &2x,"* Testing Forces *"/,2x,"******************"/)')
    if (TF_atom_moved > ni_in_cell) then
       if(inode==ionode) write (io_lun,fmt='(2x,"Error: specified atom &
            &out-of-range: ",2i8)') TF_atom_moved, ni_in_cell
       TF_atom_moved = 1
    end if
    ...
    if (flag_test_all_forces .OR. flag_which_force == 1) then
       ...
       call test_full (fixed_potential, vary_mu, n_L_iterations, &
            L_tolerance, tolerance, mu, total_energy, &
            expected_reduction)
    ...
    if (flag_test_all_forces .OR. flag_which_force == 5) then
       ...
       call test_PhiPulay_nonlocal (fixed_potential, vary_mu, &
            n_L_iterations, L_tolerance, tolerance, mu, total_energy, &
            expected_reduction)
       if (flag_test_all_forces) then
          call update_H (fixed_potential)
    ...
    if (flag_test_all_forces .OR. flag_which_force == 6) then
       ...
       call test_PhiPulay_KE (fixed_potential, vary_mu, &
            n_L_iterations, L_tolerance, tolerance, mu, total_energy, &
            expected_reduction)
       if (flag_test_all_forces) then
          call update_H (fixed_potential)
    ...
    if (flag_test_all_forces .OR. flag_which_force==7) then
       if (inode==ionode) write(io_lun,*) '*** Local (Ha, XC, loc ps) &
            &phi Pulay ***'
    ...
    if (flag_test_all_forces .OR. flag_which_force==9) then
       ...
       if (flag_test_all_forces) then
          ...
          if(flag_self_consistent) then ! Vary only DM and charge
             ...
             call FindMinDM (n_L_iterations, vary_mu, &
...
!!
  subroutine test_FullPulay (fixed_potential, vary_mu, n_L_iterations,&
    ...
    use primary_module, ONLY : bundle
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, WhichPulay, BothPulay, &
         flag_self_consistent, flag_basis_set, PAOs, blips, &
         ni_in_cell, flag_spin_polarisation
    use energy, ONLY: local_ps_energy, hartree_energy, xc_energy, &
    ...
    use mult_module, ONLY: matK, matk_dn, allocate_temp_matrix, &
    ...
    if (flag_basis_set==PAOs) then
       ...
       if(myid==0) write(io_lun,fmt='(2x,"*                           &
            &                           *")')
       if(myid==0) write(io_lun,fmt='(2x,"*    WARNING * WARNING * &
            &WARNING * WARNING * WARNING   *")')
       if(myid==0) write(io_lun,fmt='(2x,"*                           &
            &                           *")')
       if(myid==0) write(io_lun,fmt='(2x,"* With a PAO basis we &
            &calculate NL HF AND Pulay forces *")')
       if(myid==0) write(io_lun,fmt='(2x,"*                           &
            &                           *")')
       if(myid==0) write(io_lun,fmt='(2x,"*****************************&
            &***************************")')
    ...
    if (flag_spin_polarisation) then
       matKold_dn = allocate_temp_matrix (Hrange, 0)
       call matrix_sum (zero, matKold_dn, one, matK_dn)
    end if
    ...
    if (flag_spin_polarisation) then
       call matrix_sum (zero, matK_dn, one, matKold_dn)
    end if
    ! end LT 2011/11/29
    ! Store local energy
!    E0 = hartree_energy + xc_energy + local_ps_energy
    ! Find out direction and atom for displacement
    if (myid==0) write(io_lun,fmt='(2x,"Moving atom ",i5," in &
         &direction ",i2," by ",f10.6," bohr")') TF_atom_moved, &
         TF_direction,TF_delta
    F0 = p_force(TF_direction,TF_atom_moved) + &
         HF_NL_force(TF_direction, TF_atom_moved) + &
         KE_force(TF_direction,TF_atom_moved)
    if (inode==ionode) write(io_lun,fmt='(2x,"Initial energy: ",&
         &f20.12,/,2x,"Initial pulay force: ",f20.12)') E0, F0
...
!    call get_energy(total_energy)
    ...
    if (flag_spin_polarisation) then
       call matrix_sum (zero, matK_dn, one, matKold_dn)
       call free_temp_matrix (matKold_dn)
    end if
...
!    E1 = hartree_energy + xc_energy + local_ps_energy
    ...
    F1 = p_force(TF_direction,TF_atom_moved) + &
         HF_NL_force(TF_direction,TF_atom_moved) + &
         KE_force(TF_direction,TF_atom_moved)
    if (inode==ionode) write(io_lun,fmt='(2x,"Final energy      : ",  &
         &       f20.12,/,2x,"Final pulay force: ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = half*(F1+F0)
    if(inode==ionode) write(io_lun,fmt='(2x,"Numerical Force: ",      &
         &   f20.12,/,2x,"Analytic Force : ",f20.12)') &
         numerical_force, analytic_force
    if(inode==ionode) write(io_lun,fmt='(2x,"Force error: ",e20.12)') &
         numerical_force - analytic_force
...
!!
  subroutine test_HF (fixed_potential, vary_mu, n_L_iterations, &
    ...
    use primary_module, ONLY : bundle
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, flag_self_consistent, &
    ...
    use energy, ONLY: nl_energy, get_energy, local_ps_energy, &
         band_energy, kinetic_energy
    use force_module, ONLY: get_HF_force, get_HF_non_local_force, &
         HF_and_Pulay, HF
    ...
    integer :: stat
    real(double) ::  E0, F0, E1, F1, analytic_force, numerical_force, &
         electrons, electrons_up, electrons_dn, Enl0, Enl1, Fnl0, &
         Fnl1, KE0
    real(double), dimension(3,ni_in_cell) :: HF_force
    real(double), dimension(3,ni_in_cell) :: HF_NL_force
    real(double), dimension(:), allocatable :: density_out, &
         density_out_up, density_out_dn

    ! We're coming in from initial_H: assume that initial E found
    call start_timer (tmr_std_allocation)
    allocate (density_out(maxngrid), STAT=stat)
    if (stat /= 0) call cq_abort ("test_HF: failed to allocate &
         &density_out ", maxngrid, stat)
    if (flag_spin_polarisation) then
       allocate (density(maxngrid), density_out_up(maxngrid), &
            density_out_dn(maxngrid), STAT=stat)
       if (stat /= 0) call cq_abort ("test_HF: failed to allocate &
            &arrays for spin polarisation", maxngrid, stat)
       density = density_up + density_dn
    end if
    ...
    if(.NOT.flag_self_consistent) then ! Harris-Foulkes requires
       ! output density
       if (flag_spin_polarisation) then
          call get_electronic_density (density_out_up, electrons_up, &
               supportfns, H_on_supportfns, inode, ionode, maxngrid, &
               spin=1)
          call get_electronic_density (density_out_dn, electrons_dn, &
               supportfns, H_on_supportfns, inode, ionode, maxngrid, &
               spin=1)
          density_out = density_out_up + density_out_dn
       else
    ...
    ! Find out direction and atom for displacement
    if (inode == ionode) write (io_lun,fmt='(2x,"Moving atom ",i5,"   &
         &       in direction ",i2," by ",f10.6," bohr")') &
         TF_atom_moved, TF_direction,TF_delta
    Fnl0 = HF_NL_force(TF_direction,TF_atom_moved)
    if(inode == ionode) write (io_lun,fmt='(2x,"Initial NL energy: ", &
         &        f20.12,/,2x,"Initial NL force : ",f20.12)') Enl0, &
         Fnl0
    ...
    if (inode == ionode) write (io_lun,fmt='(2x,"Initial band energy: &
         &",f20.12,/,2x,         "Initial HF force : ",f20.12)') E0, &
         F0
    ! Move the specified atom 
    if (TF_direction==1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
    ...
    if (inode==ionode) write(io_lun,fmt='(2x,"Final NL energy: ",     &
         &    f20.12,/,2x,"Final NL force : ",f20.12)') Enl1, Fnl1
    numerical_force = -(Enl1-Enl0)/TF_delta
    analytic_force = half*(Fnl1+Fnl0)
    if (inode==ionode) write(io_lun,fmt='(2x,"Numerical NL Force: ",  &
         &       f20.12,/,2x,"Analytic NL Force : ",f20.12)') &
         numerical_force, analytic_force
    if (inode==ionode) write(io_lun,fmt='(2x,"Force error: ",&
         &e20.12)') numerical_force - analytic_force
    if (inode==ionode) write(io_lun,fmt='(2x,"Final band energy: ",   &
         &      f20.12,/,2x,"Final HF force : ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = half*(F1+F0)
    if (inode==ionode) write(io_lun,fmt='(2x,"Numerical local Force: &
         &",         f20.12,/,2x,"Analytic local Force : ",f20.12)') &
         numerical_force, analytic_force
    if (inode==ionode) write(io_lun,fmt='(2x,"Force error: ",         &
         &e20.12)') numerical_force - analytic_force
    ! Move the specified atom back
    ...
    if (stat /= 0) call cq_abort ("test_HF: failed to free moemory&
         & for density_out: ", stat)
    if (flag_spin_polarisation) then
       deallocate (density, density_out_up, density_out_dn, STAT=stat)
       if (stat /= 0) call cq_abort ("test_HF: failed to free moemory&
            & for spin polarisation: ", stat)
    end if
    return
...
!!
  subroutine test_PhiPulay_nonlocal (fixed_potential, vary_mu, &
    ...
    use primary_module, ONLY : bundle
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, flag_basis_set, blips, PAOs, &
         ni_in_cell, flag_spin_polarisation
    ...
    if(flag_basis_set==PAOs) then
       ...
       if(myid==0) write(io_lun,fmt='(2x,"*                           &
            &                                *")')
       if(myid==0) write(io_lun,fmt='(2x,"* With a PAO basis we DO &
    ...
    E0 = nl_energy
    ! Find out direction and atom for displacement
    if(inode==ionode) write(io_lun,fmt='(2x,"Moving atom ",i5," in &
         &direction ",i2," by ",f10.6," bohr")') TF_atom_moved, &
         TF_direction,TF_delta
    F0 = HF_NL_force(TF_direction,TF_atom_moved)
    if(inode==ionode) write(io_lun,fmt='(2x,"Initial NL energy: ",&
         &f20.12,/,2x,"Initial NL force : ",f20.12)') E0, F0
    ! Move the specified atom 
    if(TF_direction==1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
            x_atom_cell(id_glob_inv(TF_atom_moved)) + TF_delta
    ...
    F1 = HF_NL_force(TF_direction,TF_atom_moved)
    if (inode==ionode) write(io_lun,fmt='(2x,"Final NL energy: ",&
         &f20.12,/,2x,"Final NL force : ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = half*(F1+F0)
    if (inode==ionode) write(io_lun,fmt='(2x,"Numerical Force: ",&
         &f20.12,/,2x,"Analytic Force : ",f20.12)') numerical_force, &
         analytic_force
    if(inode==ionode) write(io_lun,fmt='(2x,"Force error: ",e20.12)') &
         numerical_force - analytic_force
    ! Move the specified atom back
...
!!
  subroutine test_PhiPulay_KE (fixed_potential, vary_mu, &
    ...
    use primary_module, ONLY : bundle
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, flag_basis_set, blips, ni_in_cell, &
         flag_spin_polarisation
    ...
    if(inode==ionode) write(io_lun,fmt='(2x,"Initial KE energy: ",&
         &f20.12,/,2x,"Initial KE force : ",f20.12)') E0, F0
    ! Move the specified atom 
    if(TF_direction==1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
            x_atom_cell(id_glob_inv(TF_atom_moved)) + TF_delta
    ...
    if(inode==ionode) write(io_lun,fmt='(2x,"Final KE energy: ",&
         &f20.12,/,2x,"Final KE force : ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = half*(F1+F0)
    if(inode==ionode) write(io_lun,fmt='(2x,"Numerical Force: ",&
         &f20.12,/,2x,"Analytic Force : ",f20.12)') numerical_force, &
...
!!
  subroutine test_PhiPulay_local (fixed_potential, vary_mu, &
    ...
    use primary_module, ONLY : bundle
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, WhichPulay, PhiPulay, &
         flag_basis_set, blips, PAOs, ni_in_cell, flag_spin_polarisation
    use energy, ONLY: local_ps_energy, hartree_energy, xc_energy, &
         get_energy, band_energy
...
!    E0 = hartree_energy + xc_energy + local_ps_energy
    ...
    F0 = p_force(TF_direction,TF_atom_moved)
    if(inode==ionode) write(io_lun,fmt='(2x,"Initial energy         : &
         &",f20.12,/,2x,"Initial phi Pulay force : ",f20.12)') E0, F0
    ! Move the specified atom 
    if(TF_direction==1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
            x_atom_cell(id_glob_inv(TF_atom_moved)) + TF_delta
...
!    E1 = hartree_energy + xc_energy + local_ps_energy
    ...
    F1 = p_force(TF_direction,TF_atom_moved)
    if(inode==ionode) write(io_lun,fmt='(2x,"Final energy         : ",&
         &f20.12,/,2x,"Final phi Pulay force: ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
...
!!
  subroutine test_SPulay (fixed_potential, vary_mu, n_L_iterations, &
    ...
    use primary_module, ONLY : bundle
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, flag_self_consistent, ni_in_cell
    use energy, ONLY: local_ps_energy, hartree_energy, xc_energy, &
         get_energy, band_energy
    use force_module, ONLY: pulay_force
    use GenComms, ONLY: myid, inode, ionode
    ...
    WhichPulay = SPulay
    call pulay_force (p_force, fixed_potential, vary_mu, &
         n_L_iterations, L_tolerance, tolerance, mu, &
...
!    E0 = hartree_energy + xc_energy + local_ps_energy
    ...
    F0 = p_force(TF_direction,TF_atom_moved)
    if(inode==ionode) write(io_lun,fmt='(2x,"Initial energy      : ",&
         &f20.12,/,2x,"Initial S-pulay force: ",f20.12)') E0, F0
    ! Move the specified atom 
    if(TF_direction==1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
            x_atom_cell(id_glob_inv(TF_atom_moved)) + TF_delta
...
!    E1 = hartree_energy + xc_energy + local_ps_energy
    ...
    F1 = p_force(TF_direction,TF_atom_moved)
    if(inode==ionode) write(io_lun,fmt='(2x,"Final energy      : ",&
         &f20.12,/,2x,"Final S-pulay force: ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = 0.5_double*(F1+F0)
    if(inode==ionode) write(io_lun,fmt='(2x,"Numerical Force: ",&
         &f20.12,/,2x,"Analytic Force : ",f20.12)') numerical_force, &
...
!!
  subroutine test_nonSC (fixed_potential, vary_mu, n_L_iterations, &
    ...
    use primary_module, ONLY : bundle
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, ni_in_cell, flag_spin_polarisation
    use energy, ONLY: local_ps_energy, hartree_energy, xc_energy, &
         get_energy, band_energy, delta_E_hartree, delta_E_xc
    ...
    allocate (density_out(maxngrid), STAT=stat)
    if (flag_spin_polarisation) then
       allocate (density_out_up(maxngrid), density_out_dn(maxngrid), &
            STAT=stat)
    end if
    call stop_timer (tmr_std_allocation)
    if (flag_spin_polarisation) then
       call get_electronic_density (density_out_up, electrons_up, &
            supportfns, H_on_supportfns, inode, ionode, maxngrid, spin=1)
       call get_electronic_density (density_out_dn, electrons_dn, &
            supportfns, H_on_supportfns, inode, ionode, maxngrid, spin=2)
    else
       call get_electronic_density (density_out, electrons, &
            supportfns, H_on_supportfns, inode, ionode, maxngrid)
    end if
    ! Find force
    if (flag_spin_polarisation) then
       call get_nonSC_correction_force (nonSC_force, density_out_up, &
            density_out_dn, inode, ionode, ni_in_cell, maxngrid)
    else
    ...
    ! Find out direction and atom for displacement
    if (inode == ionode) write (io_lun,fmt='(2x,"Moving atom ",i5," &
         &in direction ",i2," by ",f10.6," bohr")') TF_atom_moved, &
         TF_direction, TF_delta
    F0 = nonSC_force(TF_direction, TF_atom_moved)
    if (inode==ionode) write (io_lun,fmt='(2x,"Initial energy   : ",&
         &f20.12,/,2x,"Initial NSC force: ",f20.12)') E0, F0
    ! Move the specified atom 
    if (TF_direction == 1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
            x_atom_cell(id_glob_inv(TF_atom_moved)) + TF_delta
    ...
    deallocate (density_out, STAT=stat)
    if (flag_spin_polarisation) then
       deallocate (density_out_up, density_out_dn, STAT=stat)
    end if
    call stop_timer(tmr_std_allocation)
    E1 = band_energy + delta_E_hartree + delta_E_xc
    F1 = nonSC_force(TF_direction,TF_atom_moved)
    if (inode==ionode) write(io_lun,fmt='(2x,"Final energy   : ",&
         &f20.12,/,2x,"Final NSC force: ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = half*(F1+F0)
    if (inode==ionode) write(io_lun,fmt='(2x,"Numerical Force: ",&
         &f20.12,/,2x,"Analytic Force : ",f20.12)') numerical_force, &
...
!!
  subroutine test_full (fixed_potential, vary_mu, n_L_iterations, &
    ...
    use GenComms, ONLY: myid, inode, ionode
    use H_matrix_module, ONLY: get_H_matrix
    use global_module, ONLY: flag_self_consistent, flag_spin_polarisation
    use SelfCon, ONLY: new_SC_potl
    use DMMin, ONLY: FindMinDM
    use density_module, ONLY: density, density_up, density_dn
    ...
    call force (fixed_potential, vary_mu, n_L_iterations, L_tolerance, &
         tolerance, mu, total_energy, expected_reduction, .true.)
    ! Find out direction and atom for displacement
    if (inode == ionode) write (io_lun,fmt='(2x,"Moving atom ",i5," in &
         &direction ",i2," by ",f10.6," bohr")') TF_atom_moved, &
         TF_direction, TF_delta
    ...
    if (inode == ionode) write (io_lun, fmt='(2x,"Initial energy: ",&
         &f20.12,/,2x,"Initial force : ",f20.12)') E0, F0
    ! Move the specified atom 
    if (TF_direction == 1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
            x_atom_cell(id_glob_inv(TF_atom_moved)) + TF_delta
    ...
    call cover_update (x_atom_cell, y_atom_cell, z_atom_cell, &
         DCS_parts, parts)
    if (.NOT. flag_old_ewald) call cover_update (x_atom_cell, &
         y_atom_cell, z_atom_cell, ewald_CS, parts)
    ! Reproject blips
    call update_H (fixed_potential)
    if(flag_self_consistent) then ! Vary only DM and charge density
       reset_L = .true.
    ...
    if (inode == ionode) write(io_lun,fmt='(2x,"Final energy: ",&
         &f20.12,/,2x,"Final force : ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = half*(F1+F0)
    if (inode == ionode) write(io_lun,fmt='(2x,"Numerical Force: ",&
         &f20.12,/,2x,"Analytic Force : ",f20.12)') numerical_force, &
...
!!
  subroutine test_cdft (fixed_potential, vary_mu, n_L_iterations, &
    ...
    use global_module, ONLY: iprint_MD, x_atom_cell, y_atom_cell, &
         z_atom_cell, id_glob_inv, flag_self_consistent, ni_in_cell
    use energy, ONLY: local_ps_energy, hartree_energy, xc_energy, &
         get_energy, band_energy, cdft_energy
    use GenComms, ONLY: myid, inode, ionode
    use S_matrix_module, ONLY: get_S_matrix
    use global_module, ONLY: WhichPulay, SPulay, flag_basis_set, &
         blips, PAOs
    ...
    use density_module, ONLY: build_Becke_weight_forces, &
         build_Becke_weights, get_cdft_constraint
    use maxima_module, ONLY: maxngrid
    ...
    write(io_lun,fmt='(2x,"Moving atom ",i5," in direction ",i2," by &
         &",f10.6," bohr")') TF_atom_moved, TF_direction,TF_delta
    F0 = c_force(TF_direction,TF_atom_moved)
    if(inode==ionode) write(io_lun,fmt='(2x,"Initial cDFT energy      &
         &: ",f20.12,/,2x,"Initial cDFT force: ",f20.12)') E0, F0
    ! Move the specified atom 
    if(TF_direction==1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
            x_atom_cell(id_glob_inv(TF_atom_moved)) + TF_delta
    ...
    if(inode==ionode) write(io_lun,fmt='(2x,"Final cDFT energy      : &
         &",f20.12,/,2x,"Final cDFT force: ",f20.12)') E1, F1
    numerical_force = -(E1-E0)/TF_delta
    analytic_force = 0.5_double*(F1+F0)
    if(inode==ionode) write(io_lun,fmt='(2x,"Numerical cDFT Force: ",&
         &f20.12,/,2x,"Analytic cDFT Force : ",f20.12)') &
         numerical_force, analytic_force
    if(inode==ionode) write(io_lun,fmt='(2x,"cDFT Force error: ",&
         &e20.12)') numerical_force - analytic_force
    ! Move the specified atom back
    if(TF_direction==1) then
       x_atom_cell(id_glob_inv(TF_atom_moved)) = &
