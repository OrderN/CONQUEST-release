...
!!
  subroutine assemble_2(range,matA,flag,matAD)
    ...
    if(iprint_basis>=5) write(*,fmt='(6x,i5," Zeroing S")') myid
    call matrix_scale(zero,matA)
    !write(*,*) 'Zeroing dS: ',PRESENT(matAD)
    if(PRESENT(matAD)) call matrix_scale(zero,matAD)
    if(iprint_basis>=5) write(*,fmt='(6x,i5," Done Zeroing")') 
    iprim = 0; ip = 0
    if(flag_paos_atoms_in_cell.OR.flag==3) then
       do part = 1,bundle%groups_on_node ! Loop over primary set partitions
          if(iprint_basis>=6) write(*,fmt='(6x,"Processor, partition: ",2i6)') myid,part
          if(bundle%nm_nodgroup(part)>0) then ! If there are atoms in partition
             do memb = 1,bundle%nm_nodgroup(part) ! Loop over atoms
                ...
                if(iprint_basis>=6) write(*,'(6x,"Processor, atom, spec: ",3i6)') myid,memb,atom_spec
                do neigh = 1, mat(part,range)%n_nab(memb) ! Loop over neighbours of atom
                   ...
                   if(iprint_basis>=6) write(*,'(6x,"Processor, neighbour, spec: ",3i6)') myid,neigh,neigh_species
