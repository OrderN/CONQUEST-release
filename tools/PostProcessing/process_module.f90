...
contains
  ...
  subroutine process_pdos
    ...
    ! pDOS rotation variables
    use local, ONLY: flag_rotate_pdos, flag_rotate_pdos_mode, flag_rotate_pdos_debug, &
       pdos_ax, pdos_ay, pdos_az, rotate_pdos_atoms, axes_angles, euler_angles, find_neighbours,&
       rotate_pdos_natoms, nghbr_arr, U1, U2
    ...
    real(double), dimension(:,:), allocatable :: bond(:,:)
    ...
    if (flag_rotate_pdos) then
       ...
       if (flag_rotate_pdos_debug .and. flag_rotate_pdos_mode /= 1) then
          ...
          write(*, fmt='(/4x, "A1: ")')
          do j = 1, 3
             write(*, fmt='(/4x,3(f10.5,1X))', advance="no") A1(j, :)
          end do
          write(*, fmt='(/4x, "A2: ")')
          do j = 1, 5
             write(*, fmt='(/4x,5(f10.5,1X))', advance="no") A2(j, :)
          end do
       ...
       if (flag_rotate_pdos_mode == 0) then
          ...
          do i = 1, rotate_pdos_natoms
             ...
             write(*, fmt='(/2x,"User input angle (rad, deg) ", 2(f10.5))') angle, angle * (180.0_double / pi)
             call construct_rodrigues(axis, angle, rod)
             write(*, fmt='(/2x,"New local axes for atom ", I0, ": ")') &
                  rotate_pdos_atoms(i)
  ...
  ! pDOS rotation subroutines
  ! misc subroutine for printing out orbital rotation matrices
  subroutine print_orbital_weights(atom_index)
    ...
    write(*,*)
  end subroutine print_orbital_weights
  ...
  !!   active Euler angles about extrinsic (fixed) cell axes. Positive angle
  !!   is anticlockwise.
  !!  INPUTS
  ...
  !!    26/08/2026 - C. Xu: Fix orbital convention
  !!  SOURCE
  ...
  subroutine construct_EulerMatrices(E1, E2, atom_index)
    use datatypes
    use numbers, ONLY: pi, zero, one, two, three, quarter, half
    ...
    real(double) :: identity3(3,3), identity5(5,5)
    integer :: i
    ...
    E2(1,1) = c2a*cb*c2g - sa*ca*(c2b+three)*sg*cg
    E2(1,2) = sb*(c2a*cg - two*sa*ca*cb*sg)
    E2(1,3) = sqrt(three)*sa*ca*sb*sb
    E2(1,4) = sb*(s2a*cb*cg + c2a*sg)
    E2(1,5) = cb*c2a*s2g +quarter*s2a*(three+c2b)*c2g

    E2(2,1) = sb*(sa*cb*s2g - ca*c2g)
    E2(2,2) = ca*cb*cg - sa*c2b*sg
    E2(2,3) = sqrt(three)*sa*sb*cb
    E2(2,4) = c2b*sa*cg + ca*cb*sg
    E2(2,5) = -sb*(sa*cb*c2g +s2g*ca)

    E2(3,1) = -sqrt(three)*sb*sb*sg*cg
    E2(3,2) = sqrt(three)*sb*cb*sg
    E2(3,3) = quarter*(three*c2b+one)
    E2(3,4) = -sqrt(three)*sb*cb*cg
    E2(3,5) = half*sqrt(three)*sb*sb*c2g

    E2(4,1) = sb*(ca*cb*s2g + sa*c2g)
    E2(4,2) = -sa*cb*cg - ca*c2b*sg
    E2(4,3) = sqrt(three)*ca*sb*cb
    E2(4,4) = ca*c2b*cg - sa*cb*sg
    E2(4,5) = sb*sa*s2g - half*ca*s2b*c2g

    E2(5,1) = -quarter*c2a*(c2b + three)*s2g - c2g*cb*s2a
    E2(5,2) = -sb*(c2a*cb*sg + s2a*cg)
    E2(5,3) = half*sqrt(three)*sb*sb*c2a
    E2(5,4) = half*c2a*s2b*cg - s2a*sb*sg
    E2(5,5) = -s2a*cb*s2g + quarter*c2a*(three + c2b)*c2g
  ...
  subroutine euler_from_axisangle(axis, angle)
    ...
    write(*, fmt='(/4x,"alpha: ", (f10.5,1X))', advance="no") &
         180.0_double/pi*(datan2(axis(3)*sin(angle / two),cos(angle / two)) + datan2(axis(2), axis(1)) - (pi/two))
    write(*, fmt='(/4x,"beta: ", (f10.5,1X))',  advance="no") &
         180.0_double/pi*2*asin(sqrt(axis(1)*axis(1) + axis(2)*axis(2)) * sin(angle / two))
    write(*, fmt='(/4x,"gamma: ", (f10.5,1X))',  advance="no") &
         180.0_double/pi*(datan2(axis(3)*sin(angle/two),cos(angle/two)) - datan2(axis(2), axis(1)) + (pi/two))
  end subroutine euler_from_axisangle
  ...
  subroutine calculate_axis_angle(w1, w2, w3, axis, angle)
    ...
    use datatypes
    use numbers, ONLY: zero, pi, one, RD_ERR
    use GenComms, ONLY: cq_abort
    ...
    if (det < one - RD_ERR .or. det > one + RD_ERR) &
    ...
    if (abs(sin_angle) < RD_ERR .and. abs(cos_angle) < RD_ERR) &
  ...
  subroutine construct_rodrigues(axis, angle, matrix)
    ...
    temp = identity - (sin(angle)*KT) + (one - cos(angle))*matmul(KT, KT)
  ...
  subroutine construct_C1(rod, C1)
    ...
    use datatypes
    use numbers, ONLY: zero, one, RD_ERR
    use local, ONLY: flag_rotate_pdos_debug
    ...
    if (flag_rotate_pdos_debug) then
       ...
       identity(2, 2) = one
       identity(3, 3) = one
       if(all(abs(matmul(C1, transpose(C1)) - identity) < RD_ERR)) then
  ...
  subroutine construct_C2(r, C2)
    ...
    use datatypes
    use numbers, ONLY: half, two, four
    use local, ONLY: flag_rotate_pdos_debug
    ...
    C2(1, 5) = r(6)*r(9)*half
    C2(1, 4) = r(4)*r(7) + r(6)*r(9)*half
    ! Row 2
    C2(2, 1) = r(3)*r(8) + r(2)*r(9)
    C2(2, 2) = r(3)*r(7) + r(1)*r(9)
    C2(2, 3) = r(2)*r(7) + r(1)*r(8)
    C2(2, 5) = r(3)*r(9)*half
    C2(2, 4) = r(1)*r(7) + r(3)*r(9)*half
    ! Row 3
    C2(3, 1) = r(3)*r(5) + r(2)*r(6)
    C2(3, 2) = r(3)*r(4) + r(1)*r(6)
    C2(3, 3) = r(2)*r(4) + r(1)*r(5)
    C2(3, 5) = r(3)*r(6)*half
    C2(3, 4) = r(1)*r(4) + r(3)*r(6)*half

    ! Row 4
    C2(4, 1) = two*(r(2)*r(3) - r(5)*r(6))
    C2(4, 2) = two*(r(1)*r(3) - r(4)*r(6))
    C2(4, 3) = two*(r(1)*r(2) - r(4)*r(5))
    C2(4, 5) = half*(r(3)*r(3) - r(6)*r(6))
    C2(4, 4) = r(1)*r(1) - r(4)*r(4) + half*(r(3)*r(3) - r(6)*r(6))

    ! Row 5
    C2(5, 1) = (four*r(8)*r(9)) - two*(r(2)*r(3) + r(5)*r(6))
    C2(5, 2) = (four*r(7)*r(9)) - two*(r(1)*r(3) + r(4)*r(6))
    C2(5, 3) = (four*r(7)*r(8)) - two*(r(1)*r(2) + r(4)*r(5))
    C2(5, 5) = r(9)*r(9) - half*(r(3)*r(3) + r(6)*r(6))
    C2(5, 4) = C2(5, 5) - r(1)*r(1) - r(4)*r(4) + two*r(7)*r(7)
  ...
  subroutine rotate_coefficients
    ...
    do i = 1, size(g_atom_lookup)
       ! As in CONQUEST, m = -2 = -xy NOT xy, need to negate first row and column, except U2(1,1)
       U2(1,2:,i) = -U2(1,2:,i)
       U2(2:,1,i) = -U2(2:,1,i)
    end do
                      ...
                      do i_z = 1, nzeta
                         ...
                         case(1)
                            evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, i_band_c, i_kp, i_spin) = &
                                 matmul(U1(:,:,i_atom), evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, &
                                 i_band_c, i_kp, i_spin))
                            scaled_evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, i_band_c, i_kp, i_spin) = &
                                 matmul(U1(:,:,i_atom), scaled_evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, &
                                 i_band_c, i_kp, i_spin))
                         case(2)
                            evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, i_band_c, i_kp, i_spin) = &
                                 matmul(U2(:,:,i_atom), evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, &
                                 i_band_c, i_kp, i_spin))
                            scaled_evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, i_band_c, i_kp, i_spin) = &
                                 matmul(U2(:,:,i_atom), scaled_evec_coeff(sf_offset+1:sf_offset+norbs, g_atom, &
                                 i_band_c, i_kp, i_spin))
                         end select
                         sf_offset = sf_offset + norbs
                      end do ! i_z
  ...
  subroutine nearest_neighbours(atomno, bond)
    ...
    where (abs(temp) < RD_ERR) temp = BIG
  ...
  subroutine axes_from_nn(atomno, bond)
    ...
    if (find_neighbours(4, find_atomno) == 0) then
       ...
       call project_onto_plane(norm_bond(:, ibond_principal),norm_bond(:, idx_direction), proj_vector)
    else
       ...
       call project_onto_plane(norm_bond(:, ibond_principal),norm_bond(:, ibond_sec), proj_vector)
