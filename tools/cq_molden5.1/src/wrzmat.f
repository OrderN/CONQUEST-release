...
c
      ...
      subroutine pdbzmd(ianz,iaton,iresid,iconn)
      implicit double precision (a-h,o-z)
      parameter (mxcon=10)
      common /athlp/  iatoms, mxnat
      common /zmfrst/ ihaszm, nz, mxzat
...
100   ical = 0
     ...
     &          iconn(mxcon+1,*),ityp(*),ipdbt(*),icalf(6,*)
      ...
      if (.not.addat(iat1,iat2,iat3,ian,bl,alpha,dih,iret,
     &               ianz,iaton,iatclr,iconn,iresid,ityp,ipdbt,
     &               ncalf,icalf,coo)) then
      ...
      logical function addat(iat1,iat2,iat3,ian,bl,alpha,dih,iret,
     &                       ianz,iaton,iatclr,iconn,iresid,ityp,ipdbt,
     &                       ncalf,icalf,coo)
      implicit double precision (a-h,o-z)
      parameter (mxcon=10)
...
c idogau = 2, try to write amber type
     ...
     &                  irsnr(i),(coo(k,ihpdb(j))*toang,k=1,3)
         ...
         do i=1,iatoms
            if (iresid(i).lt.-3.and.ianz(i).ne.100) then
     ...
     &            'HETATM',iat,tocapf(elemnt(ianz(i)))//'  ',
...
c
     ...
     &          file=filenm,status='unknown',err=2000)
         ...
         elseif ((iopt.ge.1.and.iopt.le.3).or.
     &           (iopt.ge.19.and.iopt.le.21)) then
...
c========================= Map functions to subroutines ===
     ...
     &             idum1,norbs,idum2,((rdum,j=1,idum1),i=1,3)
                   ...
                   read(iun2,'(3i8)',end=1000,err=1000)
     &                  idum1,norbs,idum2
                   do i=1,idum1
                      read(iun2,'(1x,3f16.8)',end=1000,err=1000)
     &                   (rdum,j=1,3)
                   end do
                   if (idum1.gt.numatm) goto 1000 
                endif
