...
10    read(iun2,'(a)',end=300,err=300) lstr
      ilen = ifblen(lstr)
     ...
     &   goto 10
         ...
         read(iun2,'(6x,f22.15)',end=300,err=300) convt
      endif
      convg1(jend1) = convt
...
100   read(iun2,'(a)',end=110,err=400) lstr
      goto 100
110   backspace iun2
      if (g92) then
...
120   read(iun2,'(a)',end=400,err=400) lstr
      ilen = ifblen(lstr)
      if (index(lstr,'SCF Done').ne.0
     ...
     &   goto 120
         ...
         read(iun2,'(6x,f22.15)',end=400,err=400) convt
      endif
      convg2(jend2) = convt
