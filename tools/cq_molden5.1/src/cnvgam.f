...
10    if (direct) then
     ...
     &              '  cycle           total      electronic',istat)
      ...
      if (.not.direct.and.dft) then
         call readel(lstr,iels2)
      endif
      read(iun2,'(a)',end=300,err=300) lstr
...
70    call search(lstr,'cycle    time   tester               energy',
     &            istat)
      if (istat.ne.0) then
         call readel(lstr,2)
80       read(iun2,'(a)',end=300,err=300) lstr
         if (lstr(1:8).ne.'        ') then
            jend1=jend1+1
...
50    backspace iun2
      backspace iun2
      read(iun2,'(a)',end=400,err=400) lstr
      if (lstr(1:8).eq.'        ') goto 50
      backspace iun2
      jend2 = 0

100   read(iun2,'(a)',end=400,err=400) lstr
