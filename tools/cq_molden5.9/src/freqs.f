...
c     &             ((frtt(j,k),j=1,3),k=1,3)
                  call nxtlin(lstr,jstat)
                  if (jstat.eq.1.or.jstat.eq.2) goto 100
                  read(lstr,'(i6,4x,3(2x,3f7.2))',err=100,end=100) 
     &             iattmp,((frtt(j,k),j=1,3),k=1,3)
                  ...
                  call nxtlin(lstr,jstat)
                  if (jstat.eq.1.or.jstat.eq.2) goto 100
                  read(lstr,'(8x,3(2x,3f7.2))',err=100,end=100) 
     &             ((frtt(j,k),j=1,3),k=1,3)
            ...
            do i=1,iatoms
               do j=1,3
                   call nxtlin(lstr,jstat)
                   if (jstat.eq.1.or.jstat.eq.2) goto 100
                   read(lstr,'(23x,5f10.5)',err=100,end=100) 
     &                  (freqt(k),k=1,5)
                   a(j,i) = freqt(ioff)
               end do
            end do
         ...
         if (g94) then
            call redel(lstr,jatoms+ilegnd)
         else
            call redel(lstr,iatoms*3+ilegnd)
         endif
      endif
      if (g94) then
         ivibs = ivibs + 3
      ...
      if (index(lstr,tstr).ne.0) then
          ...
          do i=1,iatoms
             do j=1,3
                 ...
                 read(lstr,'(23x,9f10.5)',err=100,end=100) 
     &                (freqt(k),k=1,9)
                 a(j,i) = freqt(ioff)
             end do
          end do
     ...
     &                dolap,doelf
      ...
      if (irtype.eq.4) then
           ...
           elseif (iftyp.eq.15) then
              if (iecce.eq.1) then
                 call enwcrd(idebug,ifreq,istat)
              else
