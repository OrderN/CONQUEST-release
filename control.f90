...
!!  SOURCE
!!
  subroutine control_run (fixed_potential, vary_mu, mu, total_energy)
    ...
    if (leqi(runtype,'static')) then
       call get_E_and_F (fixed_potential, vary_mu, mu, total_energy, &
            .true., .true.)
       return
    else if (leqi(runtype,'cg')) then
       call cg_run (fixed_potential, vary_mu, mu, total_energy)
    else if(leqi(runtype,'md')) then
       call md_run (fixed_potential, vary_mu, mu, total_energy)
    else if(leqi(runtype,'pulay')) then
       call pulay_relax (fixed_potential, vary_mu, mu, total_energy)
    else if(leqi(runtype,'dummy')) then
       call dummy_run (fixed_potential, vary_mu, mu, total_energy)
    else
...
!!  SOURCE
!!
  subroutine cg_run (fixed_potential, vary_mu, mu, total_energy)

    ! Module usage
    use numbers
    use units
    use global_module, ONLY: iprint_gen, ni_in_cell, x_atom_cell, &
         y_atom_cell, z_atom_cell, id_glob, atom_coord, rcellx, &
         rcelly, rcellz, area_general, iprint_MD, IPRINT_TIME_THRES1
    ...
    if(stat/=0) call cq_abort("Error allocating cg in control: ",&
         ni_in_cell,stat)
    allocate(x_new_pos(ni_in_cell),y_new_pos(ni_in_cell),&
         z_new_pos(ni_in_cell),STAT=stat)
    if(stat/=0) call cq_abort("Error allocating _new_pos in control: &
         &",ni_in_cell,stat)
    ...
    call get_E_and_F(fixed_potential, vary_mu, mu, energy0, .true., &
         .true.)
    iter = 1
    ggold = zero
    energy1 = energy0
    do while(.NOT.done)
       ...
       if(inode==ionode.AND.iprint_MD>2) write(io_lun,*) ' CHECK :: &
            &Force Residual = ', for_conv*sqrt(gg)/ni_in_cell
       if(inode==ionode.AND.iprint_MD>2) write(io_lun,*) ' CHECK :: &
            &gamma = ', gamma
       if(CGreset) then
          if(gamma>one) then
       ...
       if(myid==0) write(io_lun,fmt='(/4x,"Atomic relaxation CG &
            &iteration: ",i5)') iter
       ggold = gg
       ...
       call safemin (x_new_pos, y_new_pos, z_new_pos, cg, energy0, &
            energy1, fixed_potential, vary_mu, mu, energy1)
       ! Output positions
...
!!  SOURCE
!!
  subroutine md_run (fixed_potential, vary_mu, mu, total_energy)

    ! Module usage
    use numbers
    use global_module, ONLY: iprint_gen, ni_in_cell, x_atom_cell, &
         y_atom_cell, z_atom_cell, area_general, flag_read_velocity, &
    ...
    ! Find energy and forces
    call get_E_and_F(fixed_potential, vary_mu, mu, energy0, .true., .false.)
       energy_md = energy0
    do iter = 1,MDn_steps
       ...
       call get_E_and_F(fixed_potential, vary_mu, mu, energy1, .true.,&
            .false.)
        energy_md=energy1
       ! Analyse forces
       g0 = dot(length,tot_force,1,tot_force,1)
...
!! SOURCE
!!
  subroutine dummy_run (fixed_potential, vary_mu, mu, total_energy)
    ...
    if (stat/=0) call cq_abort ("Error deallocating velocity in &
         &md_run: ", ni_in_cell, stat)
    return
1   format(4x,'Atom ',i4,' Position ',3f15.8)
...
!!  SOURCE
!!
  subroutine pulay_relax (fixed_potential, vary_mu, mu, total_energy)
    ...
    use global_module, ONLY: iprint_MD, ni_in_cell, x_atom_cell, &
         y_atom_cell, z_atom_cell, id_glob, atom_coord, rcellx, &
         rcelly, rcellz, area_general, flag_pulay_simpleStep
    use group_module, ONLY: parts
    use minimise, ONLY: get_E_and_F
    use move_atoms, ONLY: pulayStep, velocityVerlet, updateIndices, &
         update_atom_coord, safemin
    ...
    do while(.NOT.done)
       ...
       if(flag_pulay_simpleStep) then
          ...
          call get_E_and_F(fixed_potential, vary_mu, mu, energy1, &
               .true., .false.)
       else
          do j=1,ni_in_cell
             jj=id_glob(j)
          ...
          call safemin(x_new_pos,y_new_pos,z_new_pos,cg,energy0,energy1,&
               fixed_potential, vary_mu, mu, energy1)
       end if
