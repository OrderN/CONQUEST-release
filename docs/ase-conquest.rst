...
* ``CQ_PP_PATH``: the PAO path directory containing the ``.ion`` files.
...
which are the default. Knowing the exchange and correlation functional <XC>
...
* ``write_input()``:
    This function sets up the :ref:`input files <setting_up>`. For CONQUEST, the
    PAO basis is generated or copied according to the dictionary key/value pairs.
    The ``Conquest_input`` file containing the calculation parameters is written
    along with the ``coordinate`` file, containing the lattice vectors (in bohr) and
    atomic positions (in fractional coordinates).

* ``execute()``:
    This function executes the calculation. For CONQUEST, it launches the
    ``ASE_CONQUEST_COMMAND`` set in :ref:`the environment variables <ase_env>`.

* ``read_results()``:
    This function post-processes the output file. For CONQUEST, the ``energy``,
    ``forces``, ``stress`` and ``eigenvalues`` are extracted from the
    ``Conquest_out_ase`` output file.

.. note::
    The function ``read_results()`` operates on the ``Conquest_out_ase`` file.
