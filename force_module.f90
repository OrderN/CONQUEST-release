...
module force_module
  ...
  use timer_stdclocks_module, only: start_timer, stop_timer,&
       & tmr_std_allocation, tmr_std_matrices
  ...
  !! PURPOSE
  !!   Interface block for get_nonSC_correction_force
  !! INPUTS
  !! OUTPUT
  !! RETURN VALUE
  !! AUTHOR
  ...
  !! SOURCE
  !!
  interface get_nonSC_correction_force
     module procedure get_nonSC_correction_force_nospin
     module procedure get_nonSC_correction_force_spin
  end interface get_nonSC_correction_force
  !!*****
...
!!
  subroutine force(fixed_potential, vary_mu, n_cg_L_iterations, &
                   tolerance, con_tolerance, mu, total_energy, &
    ...
    use global_module, only: flag_self_consistent, flag_move_atom, &
                             id_glob, WhichPulay, BothPulay, PhiPulay,&
                             SPulay, flag_basis_set, PAOs, blips, &
                             ni_in_cell, iprint_MD, &
                             IPRINT_TIME_THRES2, area_moveatoms, &
                             flag_pcc_global, flag_perform_cdft, &
                             flag_dft_d2, flag_spin_polarisation
    ...
    real(double) :: tolerance, con_tolerance, mu, total_energy, &
                    expected_reduction
    ...
    real(double), dimension(:), allocatable :: density_out, &
                                               ...
                                               density_out_dn, &
                                               density_total
    real(double) :: electrons, electrons_up, electrons_dn, max_force
    ...
    if (flag_spin_polarisation) then
       ! allocate the density in density_module
       allocate(density_total(maxngrid), STAT=stat)
       if (stat /= 0) &
            call cq_abort("Error allocating density for spin polarisation: ",&
                          maxngrid, stat)
       call reg_alloc_mem(area_moveatoms, maxngrid, type_dbl)
       density_total = density_up + density_dn
    end if
    ...
    ! This ASSUMES that H_on_supportfns and H_dn_on_supportfns
    ! contains the values of H acting on support functions
    call start_timer(tmr_l_tmp1, WITH_LEVEL)
    call pulay_force(p_force, fixed_potential, vary_mu, &
                     n_cg_L_iterations, tolerance, con_tolerance, mu, &
    ...
    if (.not. flag_self_consistent) then
       ...
       if (stat /= 0) &
            call cq_abort ("Error allocating output density: ", maxngrid)
       call reg_alloc_mem(area_moveatoms, maxngrid, type_dbl)
       if (flag_spin_polarisation) then
          allocate(density_out_up(maxngrid), density_out_dn(maxngrid), &
                   STAT=stat)
          if (stat /= 0) &
               call cq_abort("Error allocating density_out_up and &
                              &density_out_dn: ", maxngrid)
          call reg_alloc_mem(area_moveatoms, 2 * maxngrid, type_dbl)
       end if
       call stop_timer(tmr_std_allocation)
       call start_timer(tmr_l_tmp1,WITH_LEVEL)
       if (flag_spin_polarisation) then
          ! H_on_supportfns are used as a temperary working array
          call get_electronic_density(density_out_up, electrons_up, &
                                      supportfns, H_on_supportfns, &
                                      inode, ionode, maxngrid, spin=1)
          call get_electronic_density(density_out_dn, electrons_dn, &
                                      supportfns, H_on_supportfns, &
                                      inode, ionode, maxngrid, spin=2)
          electrons = electrons_up + electrons_dn
          ! get total output density
       ...
       if (flag_spin_polarisation) then
          ...
          if (stat /= 0) &
               call cq_abort("Error deallocating density_out_up and &
                              &density_out_dn: ", maxngrid)
          call reg_dealloc_mem(area_moveatoms, 2 * maxngrid, type_dbl)
       end if
       call stop_timer(tmr_std_allocation)
    else ! for SCF
       ! Local HF force
    ...
    if (non_local) then
       ...
       call stop_print_timer(tmr_l_tmp1, &
                             "nonlocal pseudopotential force", &
                             IPRINT_TIME_THRES2)
    else
       HF_NL_force = zero
    end if
    ...
    if (flag_spin_polarisation) then
       ...
       if (stat /= 0) &
            call cq_abort("Error deallocating density: ", maxngrid, stat)
       call reg_dealloc_mem(area_moveatoms, maxngrid, type_dbl)
    end if
    call stop_timer(tmr_std_allocation)
    return
...
!!
  subroutine pulay_force(p_force, fixed_potential, vary_mu,      &
                         n_cg_L_iterations, L_tol, self_tol, mu, &
    ...
    use mult_module, only: LNV_matrix_multiply, matM12, matM12_dn, &
                           ...
                           scale_matrix_value
    use global_module, only: iprint_MD, WhichPulay, BothPulay,        &
                             PhiPulay, SPulay, flag_basis_set, blips, &
                             PAOs, sf, flag_onsite_blip_ana,          &
    ...
    integer :: n_atoms
    integer :: n_cg_L_iterations
    real(double) :: mu, L_tol, self_tol, total_energy, &
                    expected_reduction
    real(double), dimension(3, n_atoms) :: p_force

    ! Local variables
    logical :: test
    integer :: direction, count, nb, na, nsf1, point, place, i, neigh,&
               ist, wheremat, jsf, gcspart, tmp_fn, n1, n2, this_nsf

    real(double) :: electrons, energy_in, t0, t1, electrons_up, &
                    electrons_dn, energy_up, energy_dn
    ...
    if (test) then
       ...
       if (flag_spin_polarisation) then
          ...
          call get_electronic_density(density_dn, electrons_dn,        &
                                      supportfns, H_on_supportfns,     &
                                      inode, ionode, maxngrid, spin=2)
          ! H_on_supportfns and H_dn_on_supportfns are given the
          ! correct values after calls of get_H_matrix
          ...
          if (.not. diagon) then
             call LNV_matrix_multiply(electrons_up, energy_up, dontK,  &
                                      doM1, doM2, dontM3, dontM4,      &
                                      dontphi, dontE, matM12, 0, 0, 0, &
                                      spin=1)
             call LNV_matrix_multiply(electrons_dn, energy_dn, dontK,  &
                                      doM1, doM2, dontM3, dontM4,      &
                                      dontphi, dontE, matM12_dn, 0, 0, &
                                      0, spin=2)
          ...
          if (.not. diagon) &
               call LNV_matrix_multiply(electrons, energy_in, dontK, &
                                        doM1, doM2, dontM3, dontM4,  &
                                        dontphi, dontE, matM12, 0, 0, 0)
       end if ! if (flag_spin_polarisation)
    end if ! if (test)
    ...
    if (.not. diagon) then
       if (flag_spin_polarisation) then
          ...
          call LNV_matrix_multiply(electrons_dn, energy_dn, dontK,     &
                                   doM1, doM2, dontM3, dontM4,         &
                                   dontphi, dontE, matM12_dn, 0, 0, 0, &
                                   spin=2)
       else
          call LNV_matrix_multiply(electrons, energy_in, dontK, doM1,  &
    ...
    t1 = mtime()
    t0 = t1
    ! N.B. IT IS VITAL TO HAVE H_on_supportfns and H_dn_on_support fns
    ! storing the H (and corresponding H_dn) acting on support
    ! functions!  
    ! now we can evaluate support_gradient (into H_on_supportfns) If
    ! we are doing analytic on-site integrals, then zero the elements
    ! of M12 (the energy matrix) first
    if (flag_basis_set == blips .and. flag_onsite_blip_ana) then
       ...
       do np = 1, bundle%groups_on_node
          do ni = 1, bundle%nm_nodgroup(np)
             ...
             do n1 = 1, this_nsf
                do n2 = 1, this_nsf
                   call scale_matrix_value(matM12, np, ni, iprim, 0, &
                                           n1, n2, zero, 1)
                   if (flag_spin_polarisation) then
                      call scale_matrix_value(matM12_dn, np, ni, &
    ...
    if (inode == ionode .and. iprint_MD > 3)&
         write (io_lun, fmt='(4x,"get_support_gradient time: ",f12.5)') t1 - t0
    t0 = t1
    ...
    if (WhichPulay == BothPulay .or. WhichPulay == PhiPulay .or. &
       ...
       do direction = 1, 3
          ...
          ! H_on_support now stores the gradient of TOTAL E with
          ! respect to supportfuns
          call get_matrix_elements_new(inode-1, rem_bucket(sf_sf_rem),&
                                       mat_tmp, H_on_supportfns, tmp_fn)
          t1 = mtime()
          if (inode == ionode .and. iprint_MD > 3) &
             ...
             if (bundle%nm_nodgroup(np) > 0) then
                do ni = 1, bundle%nm_nodgroup(np)
                   ...
                   do neigh = 1, mat(np,Srange)%n_nab(ni)
                      ...
                      ! matM12 here is just used to work out the
                      ! position in matrix, this position will be
                      ! identical for matM12_dn, so only matM12 needed
                      ! here
                      wheremat = matrix_pos(matM12, iprim, &
                      ...
                      if (wheremat /= mat(np,Srange)%onsite(ni)) then
                         do isf = 1, mat(np,Srange)%ndimj(ist)
                            do jsf = 1, mat(np,Srange)%ndimi(ni)
                               if (flag_spin_polarisation) then
                                  ! get matM12_value = matM12_up + matM12_dn
                                  matM12_value = &
                                       return_matrix_value(matM12, np, ni, &
                                                           iprim, neigh, jsf, isf)
                                  matM12_value = matM12_value + &
                                       return_matrix_value(matM12_dn, np, ni, &
                                                           iprim, neigh, jsf, isf)
                               else
                                  ! we need factor of two for spin
                                  ! non-polarised case
                                  matM12_value = two * &
                                       return_matrix_value(matM12, np, ni, &
                                                           iprim, neigh, jsf, isf)
                               end if
                               ! the factor of two comes from chain
                               ! rule in derivatives of K respect to
                               ! phi
                               p_force(direction, i) = &
                                    p_force(direction, i) + &
                                    two * matM12_value * &
...
!!
  subroutine get_HF_non_local_force (HF_NL_force, what_force, n_atoms)
    ...
    use mult_module, only: SP_trans, mult, H_SP_SP, matrix_product, &
                           matrix_scale, matrix_transpose,          &
                           allocate_temp_matrix, free_temp_matrix,  &
                           matU, matU_dn, matUT, matUT_dn, matCS,   &
                           matSC, matK, matK_dn
    use dimens
    ...
    use global_module, only: iprint_MD, flag_basis_set, blips, PAOs, &
                             sf, nlpf, flag_spin_polarisation,       &
    ...
    use functions_on_grid, only: allocate_temp_fn_on_grid,         &
                                 free_temp_fn_on_grid, supportfns, &
    ...
    do direction = 1, 3
       ...
       if ((flag_basis_set == blips) .and. flag_analytic_blip_int) then
          ...
          call get_matrix_elements_new (inode-1,&
               & rem_bucket(sf_nlpf_rem), matdSC(direction),&
               & supportfns, dpseudofns)
          call matrix_transpose (matdSC(direction), matdCS(direction))
          call matrix_scale (-one, matdCS(direction))
          ! and the overlap between the derivatives of the support and
          ! the core functions, H_on_supportfns is used as a work array
    ...
    if (flag_spin_polarisation) then
       ...
       ! Find the transpose of U_dn (= K_dn.SC)
       call matrix_transpose (matU_dn, matUT_dn)
    else
       call matrix_product (matK, matCS, matU, mult(H_SP_SP))
    ...
    if (what_force == Pulay .OR. what_force == HF_and_Pulay) then
       do k = 1, 3
          call matrix_diagonal (matdSC(k), matU, HF_NL_force(k,:),&
               & SPrange, inode)
          if (flag_spin_polarisation) then
             ! add contribution from spin down component. Note that
             ! matrix_diagonal accumulates HF_NL_force(k,:)
             call matrix_diagonal (matdSC(k), matU_dn,&
    ...
    if (what_force == HF .OR. what_force == HF_and_Pulay) then
       do k = 1, 3
          call matrix_diagonal (matdCS(k), matUT, HF_NL_force(k,:),&
               & PSrange,inode)
          if (flag_spin_polarisation) then
             call matrix_diagonal (matdCS(k), matUT_dn,&
...
!!
  subroutine get_KE_force (KE_force, n_atoms)
    ...
    use cover_module, only: BCS_parts
    use mult_module, only: allocate_temp_matrix, free_temp_matrix,&
         & return_matrix_value, matK, matK_dn, matrix_pos
    use GenBlas
    ...
    use global_module, only: iprint_MD, flag_basis_set, blips, PAOs,&
         & sf, flag_onsite_blip_ana, flag_spin_polarisation
    use build_PAO_matrices, only: assemble_deriv_2
    ...
    !     local variables
    integer :: i, j, grad_direction, force_direction, element, np,&
         & nn, atom, n1, n2
                               ...
                               do n1=1,mat(np,Hrange)%ndimj(ist)
                                  do n2=1,mat(np,Hrange)%ndimi(i)
                                     KE_force(force_direction,atom) = KE_force(force_direction,atom) + &
                                          return_matrix_value(matK,np,i,iprim,j,n2,n1)* &
                                          return_matrix_value(mat_grad_T,np,i,iprim,j,n2,n1)
                                     KE_force(force_direction,atom) = KE_force(force_direction,atom) + &
                                          return_matrix_value(matK_dn,np,i,iprim,j,n2,n1)* &
                                          return_matrix_value(mat_grad_T,np,i,iprim,j,n2,n1)
                                  end do ! n2
                               end do
                            else
...
!!  SOURCE
!!
  subroutine get_nonSC_correction_force_nospin(hf_force, density_out, &
                                               inode, ionode, n_atoms,&
                                               size)
    ...
    use global_module, only: rcellx, rcelly, rcellz, id_glob, &
                             ...
                             functional_gga_pbe96_r99, area_moveatoms,&
                             IPRINT_TIME_THRES3, flag_pcc_global
    use block_module, only: nx_in_block, ny_in_block, nz_in_block, &
                            n_pts_in_block
    use group_module, only: blocks, parts
    use primary_module, only: domain
    ...
    use timer_module, only: cq_timer, start_timer, stop_timer, &
                            print_timer, stop_print_timer, WITH_LEVEL,&
                            TIME_ACCUMULATE_NO, TIME_ACCUMULATE_YES
    ...
    real(double), dimension(:, :) :: hf_force 
    real(double), dimension(size) :: density_out
    ...
    real(double) :: derivative, h_energy, rx, ry, rz, r2, r_from_i, x,&
                    y, z, step, pot_here
    real(double) :: dcellx_block, dcelly_block, dcellz_block
    real(double) :: dcellx_grid, dcelly_grid, dcellz_grid
    ...
    real(double) :: pcc_cutoff, pcc_cutoff2, step_pcc, x_pcc, y_pcc, &
                    z_pcc, derivative_pcc, v_pcc, fx_pcc, fy_pcc, &
                    fz_pcc, pot_here_pcc
    ...
    ! only for GGA with P.C.C.
    real(double), allocatable, dimension(:) :: density_out_GGA
    ...
    h_potential = zero
    potential = zero
    call hartree(density, h_potential, size, h_energy)
    ...
    if (flag_pcc_global) then
       ...
       if (stat /= 0) &
            call cq_abort ('Error allocating wk_grid in &
                            &get_nonSC_correction ', size, stat)
       ...
       if ((flag_functional_type == functional_gga_pbe96) .or. &
          ...
          if (stat /= 0) &
               call cq_abort('Error allocating density_out_GGA in &
                              &get_nonSC_force ', size, stat)
          call reg_alloc_mem(area_moveatoms, size, type_dbl)
    ...
    case (functional_lda_pz81)
       ! h_potential is now used as a work array storing dxc_potential
       if (flag_pcc_global) then
          call get_dxc_potential(wk_grid, h_potential, size)
       else
          call get_dxc_potential(density, h_potential, size)
       end if
    case (functional_lda_gth96)
       if (flag_pcc_global) then
          call get_GTH_dxc_potential(wk_grid, h_potential, size)
       else
          call get_GTH_dxc_potential(density, h_potential, size)
       end if
    case (functional_lda_pw92)
       if (flag_pcc_global) then
          call get_dxc_potential_LDA_PW92(wk_grid, h_potential, size)
       else
          call get_dxc_potential_LDA_PW92(density, h_potential, size)
       end if
    case (functional_gga_pbe96) ! Original PBE
       if (flag_pcc_global) then
          call get_dxc_potential_GGA_PBE(wk_grid, density_out_GGA,&
                                         potential, size)
       else
          call get_dxc_potential_GGA_PBE(density, density_out, &
                                         potential, size)
       end if
    case (functional_gga_pbe96_rev98)
       ! PBE with kappa of PRL 80, 890 (1998)
       if (flag_pcc_global) then
          call get_dxc_potential_GGA_PBE(wk_grid, density_out, &
                                         potential, size, &
                                         functional_gga_pbe96_rev98)
       else
          call get_dxc_potential_GGA_PBE(density, density_out, &
    ...
    if (flag_pcc_global)  then
       if ((flag_functional_type == functional_gga_pbe96) .or. &
          ...
          if (stat /= 0) &
               call cq_abort ('Error deallocating density_out_GGA in &
    ...
    if (flag_pcc_global .and. &
        (flag_functional_type /= functional_gga_pbe96) .and. &
    ...
    if ((flag_functional_type /= functional_gga_pbe96) .and. &
        ...
        (flag_functional_type /= functional_gga_pbe96_r99)) then
       do i = 1, n_my_grid_points
          ! contribution from n^PAD * mu'(n^PAD)
    ...
    h_potential = zero
    call hartree(density_out, h_potential, size, h_energy)
    call axpy(size, -one, h_potential, 1, potential, 1)
    ...
    if (flag_pcc_global) then
       potential = zero ! initialisation
       if ((flag_functional_type /= functional_gga_pbe96) .and. &
            (flag_functional_type /= functional_gga_pbe96_rev98) .and. &
            (flag_functional_type /= functional_gga_pbe96_r99)) then
          do i = 1, n_my_grid_points
             ! (n_{in} - n_{out}) * dxc_potential
             potential(i) = (density(i) - density_out(i)) * wk_grid(i)
          end do
          ...
          do i = 1, n_my_grid_points
             ...
             potential(i) = wk_grid(i) - h_potential_in(i) 
          end do
       end if
   ...
   return
 end subroutine get_nonSC_correction_force_nospin
 !!***

!!****f* force_module/get_nonSC_correction_force_spin
!! PURPOSE
!! INPUTS
!! OUTPUT
!! RETURN VALUE
...
!! SOURCE
!!
  subroutine get_nonSC_correction_force_spin(hf_force, density_out_up,&
                                             density_out_dn, inode, &
                                             ionode, n_atoms, size)
    ...
    use global_module, only: rcellx, rcelly, rcellz, id_glob, &
                             ...
                             functional_gga_pbe96_rev98, &
                             functional_gga_pbe96_r99, &
                             functional_lsda_pw92, area_moveatoms, &
                             IPRINT_TIME_THRES3, flag_pcc_global
    use block_module, only: nx_in_block, ny_in_block, nz_in_block, &
                            n_pts_in_block
    use group_module, only: blocks, parts
    ...
    use dimens, only: grid_point_volume, n_my_grid_points
    use GenBlas, only: axpy
    use density_module, only: density_up, density_dn, density_scale_up, &
                              density_pcc, density_scale_dn
    use hartree_module, only: hartree
    ...
    integer :: n_atoms, size
    integer :: inode, ionode
    real(double), dimension(size) :: density_out_up
    real(double), dimension(size) :: density_out_dn
    real(double), dimension(:, :) :: hf_force
    ...
    real(double) :: derivative, h_energy, rx, ry, rz, r2, r_from_i, x,&
                    y, z, step, pot_here, pot_here_up, pot_here_dn
    real(double) :: dcellx_block, dcelly_block, dcellz_block
    real(double) :: dcellx_grid, dcelly_grid, dcellz_grid
    real(double) :: xatom, yatom, zatom
    real(double) :: fx_1_up, fx_1_dn, fy_1_up, fy_1_dn, fz_1_up, fz_1_dn
    real(double) :: xblock, yblock, zblock
    real(double) :: dx, dy, dz, loc_cutoff, loc_cutoff2, electrons, v
    real(double) :: pcc_cutoff, pcc_cutoff2, step_pcc, x_pcc, y_pcc, &
                    z_pcc, derivative_pcc, v_pcc, fx_pcc_up, &
                    fx_pcc_dn, fy_pcc_up, fy_pcc_dn, fz_pcc_up, &
                    fz_pcc_dn, pot_here_pcc, pot_here_pcc_up, &
                    pot_here_pcc_dn
    logical :: range_flag
    type(cq_timer) :: tmr_l_tmp1, tmr_l_tmp2
    real(double), dimension(size) :: h_potential, density_total, &
                                     density_out_total, &
                                     dVxc_up_drho_up, dVxc_up_drho_dn,&
                                     dVxc_dn_drho_up, dVxc_dn_drho_dn
    ! only for GGA with P.C.C. 
    real(double), allocatable, dimension(:) :: h_potential_in, &
                                               wk_grid, wk_grid_up, &
                                               wk_grid_dn, &
    ...
    h_potential = zero
    density_total = density_up + density_dn
    density_out_total = density_out_up + density_out_dn
    potential_up = zero
    potential_dn = zero
    call hartree(density_total, h_potential, size, h_energy)
    call axpy(size, one, h_potential, 1, potential_up, 1)
    call axpy(size, one, h_potential, 1, potential_dn, 1)
    call stop_timer(tmr_l_tmp2, TIME_ACCUMULATE_NO)
    
    ! for P.C.C.
    if (flag_pcc_global) then
       allocate(wk_grid(size), wk_grid_up(size), wk_grid_dn(size), &
                STAT=stat)
       if (stat /= 0) &
            call cq_abort('Error allocating wk_grids in &
                           &get_nonSC_correction ', size, stat)
       call reg_alloc_mem(area_moveatoms, 3 * size, type_dbl)
       wk_grid_up = density_up + half * density_pcc
       wk_grid_dn = density_dn + half * density_pcc
       wk_grid = wk_grid_up + wk_grid_dn
       ! only for GGA
       if ((flag_functional_type == functional_gga_pbe96) .or. &
           (flag_functional_type == functional_gga_pbe96_rev98) .or. &
           (flag_functional_type == functional_gga_pbe96_r99)) then
          allocate(density_out_GGA(size), density_out_GGA_up(size), &
                   density_out_GGA_dn(size), STAT=stat)
          if (stat /= 0)&
               call cq_abort ('Error allocating &
                               &density_out_GGAs in get_nonSC_force ', &
                               size, stat)
          call reg_alloc_mem(area_moveatoms, 3 * size, type_dbl)
          density_out_GGA_up = density_out_up + half * density_pcc
          density_out_GGA_dn = density_out_dn + half * density_pcc
          density_out_GGA = density_out_GGA_up + density_out_GGA_dn
          ! copy hartree potential
          allocate(h_potential_in(size), STAT=stat)
          if (stat /= 0) &
               call cq_abort('Error allocating h_potential_in in &
    ...
    case (functional_lsda_pw92)
       ...
       if (flag_pcc_global) then
          call get_dxc_potential_LSDA_PW92(wk_grid_up, wk_grid_dn,&
                                           dVxc_up_drho_up, &
                                           dVxc_up_drho_dn, &
                                           dVxc_dn_drho_up, &
                                           dVxc_dn_drho_dn, size)
       else
          call get_dxc_potential_LSDA_PW92(density_up, density_dn,&
                                           dVxc_up_drho_up, &
                                           dVxc_up_drho_dn, &
                                           dVxc_dn_drho_up, &
                                           dVxc_dn_drho_dn, size)
    ...
    if (flag_pcc_global)  then
       if ((flag_functional_type == functional_gga_pbe96) .or. &
           (flag_functional_type == functional_gga_pbe96_rev98) .or. &
           (flag_functional_type == functional_gga_pbe96_r99)) then
          deallocate(density_out_GGA, density_out_GGA_up, &
                     density_out_GGA_dn, STAT=stat)
          if (stat /= 0) &
               call cq_abort('Error deallocating density_out_GGAs in &
                              &get_nonSC_force ', stat)
          call reg_dealloc_mem(area_moveatoms, 3 * size, type_dbl)
       end if
    end if !flag_pcc_global
    ...
    if ((flag_functional_type /= functional_gga_pbe96) .and. &
       ...
       do i = 1, n_my_grid_points
          ...
          potential_up(i) = potential_up(i) + &
                            (density_up(i) - density_out_up(i)) * &
                            dVxc_up_drho_up(i)
          potential_up(i) = potential_up(i) + &
                            (density_dn(i) - density_out_dn(i)) * &
                            dVxc_dn_drho_up(i)
          potential_dn(i) = potential_dn(i) + &
                            (density_up(i) - density_out_up(i)) * &
                            dVxc_up_drho_dn(i)
          potential_dn(i) = potential_dn(i) + &
                            (density_dn(i) - density_out_dn(i)) * &
                   ...
                   do iy = 1, ny_in_block
                      do ix = 1, nx_in_block
                         ...
                         if (igrid > n_my_grid_points) &
                              call cq_abort('get_nonSC_force: igrid error ',&
                                            igrid,n_my_grid_points)
                         pot_here_up = potential_up(igrid) * grid_point_volume
                         pot_here_dn = potential_dn(igrid) * grid_point_volume
                         dx = dcellx_grid * (ix - 1)
                         dy = dcelly_grid * (iy - 1)
                         dz = dcellz_grid * (iz - 1)
                         rx = xblock + dx - xatom
                         ...
                         if (r2 < loc_cutoff2) then
                            ...
                            fx_1_up = -x * derivative * half * density_scale_up
                            fx_1_dn = -x * derivative * half * density_scale_dn
                            fy_1_up = -y * derivative * half * density_scale_up
                            fy_1_dn = -y * derivative * half * density_scale_dn
                            fz_1_up = -z * derivative * half * density_scale_up
                            fz_1_dn = -z * derivative * half * density_scale_dn
                         else
                            fx_1_up = zero
                            fx_1_dn = zero
                            fy_1_up = zero
    ...
    ! compute - int d^3r ( delta n_{v} * dxc(n_{c} + n_{v} ) * dn_{c} )
    if (flag_pcc_global) then
       ! LT 2011/11/15: for spin polarisation case, the
       ! implementation for LDA and GGA in this case is the same?
       potential_up = zero
       potential_dn = zero
       do i = 1, n_my_grid_points
          potential_up(i) = (density_up(i) - density_out_up(i)) * &
                            dVxc_up_drho_up(i)
          potential_up(i) = potential_up(i) + &
                            (density_dn(i) - density_out_dn(i)) * &
                            dVxc_dn_drho_up(i)
          potential_dn(i) = (density_up(i) - density_out_up(i)) * &
                            dVxc_up_drho_dn(i)
          potential_dn(i) = potential_dn(i) + &
                            (density_dn(i) - density_out_dn(i)) * &
                            dVxc_dn_drho_dn(i)
                      ...
                      do iy = 1, ny_in_block
                         do ix = 1, nx_in_block
                            ...
                            if (igrid > n_my_grid_points) &
                                 call cq_abort('get_nonSC_force: igrid error ',&
                                               igrid, n_my_grid_points)
                            pot_here_pcc_up = potential_up(igrid) * &
                                              grid_point_volume
                            pot_here_pcc_dn = potential_dn(igrid) * &
                                              grid_point_volume
                            dx = dcellx_grid * (ix - 1)
                            dy = dcelly_grid * (iy - 1)
                            dz = dcellz_grid * (iz - 1)
                            ...
                            if (r2 < pcc_cutoff2) then
                               ...
                               if (range_flag) &
                                    call cq_abort('get_nonSC_force: &
                                                  &overrun problem')
                               fx_pcc_up = -x_pcc * derivative_pcc * &
                                           half * density_scale_up
                               fx_pcc_dn = -x_pcc * derivative_pcc * &
                                           half * density_scale_dn
                               fy_pcc_up = -y_pcc * derivative_pcc * &
                                           half * density_scale_up
                               fy_pcc_dn = -y_pcc * derivative_pcc * &
                                           half * density_scale_dn
                               fz_pcc_up = -z_pcc * derivative_pcc * &
                                           half * density_scale_up
                               fz_pcc_dn = -z_pcc * derivative_pcc * &
                                           half * density_scale_dn
                            else
                               fx_pcc_up = zero
                               fx_pcc_dn = zero
                            ...
                            ! components)
                            HF_force(1, ig_atom) = HF_force(1, ig_atom) + &
                                                   fx_pcc_up * pot_here_pcc_up
                            HF_force(1, ig_atom) = HF_force(1, ig_atom) + &
                                                   fx_pcc_dn * pot_here_pcc_dn
                            HF_force(2, ig_atom) = HF_force(2, ig_atom) + &
                                                   fy_pcc_up * pot_here_pcc_up
                            HF_force(2, ig_atom) = HF_force(2, ig_atom) + &
                                                   fy_pcc_dn * pot_here_pcc_dn
                            HF_force(3, ig_atom) = HF_force(3, ig_atom) + &
                                                   fz_pcc_up * pot_here_pcc_up
                            HF_force(3, ig_atom) = HF_force(3, ig_atom) + &
                                                   fz_pcc_dn * pot_here_pcc_dn
                         end do !ix
                      end do  !iy
                   end do   !iz
                end do ! naba_atoms
   ...
   call start_timer(tmr_std_allocation)
   if (flag_pcc_global) then
      deallocate(wk_grid, wk_grid_up, wk_grid_dn, STAT=stat)
      if (stat /= 0) &
           call cq_abort('Error deallocating wk_grid in &
                          &get_nonSC_correction_force ', stat)
      call reg_dealloc_mem(area_moveatoms, 3 * size, type_dbl)
   ...
   return
  end subroutine get_nonSC_correction_force_spin
!!*****
...
!!
  subroutine get_pcc_force (pcc_force, inode, ionode, n_atoms, size)
    ...
    use GenComms, only: gsum
    use global_module, only: rcellx, rcelly, rcellz, id_glob, &
         ni_in_cell, species_glob, dens, flag_functional_type, &
         ...
         area_moveatoms, IPRINT_TIME_THRES3, flag_spin_polarisation, &
    ...
    use density_module, only: density, density_up, density_dn, &
         density_scale, density_pcc
    use H_matrix_module, only: get_xc_potential, get_GTH_xc_potential,&
         get_xc_potential_LDA_PW92, get_xc_potential_GGA_PBE, &
    ...
    real(double) :: derivative_pcc, xc_energy, rx, ry, rz, r2, &
         r_from_i, x_pcc, y_pcc, z_pcc, step_pcc, pot_here_pcc, &
         pot_here_pcc_up, pot_here_pcc_dn
    real(double):: dcellx_block,dcelly_block,dcellz_block
    real(double):: dcellx_grid, dcelly_grid, dcellz_grid
    ...
    real(double), allocatable, dimension(:) :: xc_potential, &
         xc_potential_up, xc_potential_dn
    real(double), allocatable, dimension(:) :: xc_epsilon ! not used
    ! directly
    real(double), allocatable, dimension(:) :: density_wk, &
         density_wk_up, density_wk_dn
    ...
    if (stat/=0) call cq_abort ('Error allocating xc_potential and&
         & xc_epsilon in get_pcc_force ', size, stat)
    call reg_alloc_mem (area_moveatoms, 2*size, type_dbl)
    xc_potential = zero
    xc_epsilon = zero
    if (flag_spin_polarisation) then
       allocate (xc_potential_up(size), xc_potential_dn(size), STAT=stat)
       if (stat/=0) call cq_abort ('Error allocating xc_potential_up&
            & and dn in get_pcc_force ', size, stat)
       call reg_alloc_mem (area_moveatoms, 2*size, type_dbl)
       xc_potential_up = zero
       xc_potential_dn = zero
    end if
    call stop_timer (tmr_std_allocation)
    call start_timer (tmr_l_tmp2) 
    ...
    if (stat /= 0) call cq_abort ('Error allocating density_wk in&
         & get_pcc_force', size, stat)
    call reg_alloc_mem (area_moveatoms, size, type_dbl)
    if (flag_spin_polarisation) then
       allocate (density_wk_up(size), density_wk_dn(size), STAT=stat)
       if (stat /= 0) call cq_abort ('Error allocating density_wk_up&
            & and density_wk_dn in get_pcc_force', size, stat)
       call reg_alloc_mem (area_moveatoms, 2*size, type_dbl)
    end if
    
    if (flag_spin_polarisation) then
       density_wk_up = density_up + half * density_pcc
       density_wk_dn = density_dn + half * density_pcc
       ! get the total density
       density_wk = density_wk_up + density_wk_dn
    else
       ! density is already total density for spin non-polarised calculation
    ...
    if (flag_spin_polarisation) then
       select case (flag_functional_type)
       case (functional_lsda_pw92)
          call get_xc_potential_LSDA_PW92 (density_wk_up, &
               density_wk_dn, xc_potential_up, xc_potential_dn, &
       ...
       select case (flag_functional_type)
       case (functional_lda_pz81)
          call get_xc_potential (density_wk, xc_potential, xc_epsilon, &
               xc_energy, size)
       case (functional_lda_gth96)
          call get_GTH_xc_potential (density_wk, xc_potential, &
               xc_epsilon, xc_energy, size)
       case (functional_lda_pw92)
          call get_xc_potential_LDA_PW92 (density_wk, xc_potential, &
               xc_epsilon, xc_energy, size)
    ...
    if (stat /= 0) call cq_abort ("Error deallocating xc_epsilon:&
         & ", size, stat)
    call reg_dealloc_mem (area_moveatoms, size, type_dbl)
                ...
                do iz=1,nz_in_block
                   do iy=1,ny_in_block
                      do ix=1,nx_in_block
                         ...
                         if (igrid > n_my_grid_points)&
                              & call cq_abort ('get_nonSC_force:&
                              & igrid error ', igrid, n_my_grid_points)
                         ...
                         if(r2 < pcc_cutoff2) then
                            ...
                            call dsplint (step_pcc,&
                                 ...
                                 & range_flag)
                            if (range_flag) call&
                                 & cq_abort('get_pcc_force: overrun&
                                 & problem')
                         ...
                         if (flag_spin_polarisation) then
                            ! assuming that the pcc density for both
                            ! spin components are same and their
                            ! derivatives are equal to half * fx_pcc
                            ! etc
                            pcc_force(1,ig_atom) = pcc_force(1,ig_atom)&
                                 & + half * fx_pcc * pot_here_pcc_up
                            pcc_force(1,ig_atom) = pcc_force(1,ig_atom)&
                                 & + half * fx_pcc * pot_here_pcc_dn
                            pcc_force(2,ig_atom) = pcc_force(2,ig_atom)&
                                 & + half * fy_pcc * pot_here_pcc_up
                            pcc_force(2,ig_atom) = pcc_force(2,ig_atom)&
                                 & + half * fy_pcc * pot_here_pcc_dn
                            pcc_force(3,ig_atom) = pcc_force(3,ig_atom)&
                                 & + half * fz_pcc * pot_here_pcc_up
                            pcc_force(3,ig_atom) = pcc_force(3,ig_atom)&
                                 & + half * fz_pcc * pot_here_pcc_dn
                         else
                            pcc_force(1,ig_atom) = pcc_force(1,ig_atom)&
                                 & + fx_pcc * pot_here_pcc 
                            pcc_force(2,ig_atom) = pcc_force(2,ig_atom)&
    ...
    if (stat/=0) call cq_abort('Error deallocating xc_potential in&
         & get_pcc_force ',stat)
    call reg_dealloc_mem (area_moveatoms, size, type_dbl)
    deallocate (density_wk, STAT=stat)
    if (stat/=0) call cq_abort ('Error deallocating density_wk in&
         & get_pcc_force ',stat)
    call reg_dealloc_mem (area_moveatoms, size, type_dbl)
    if (flag_spin_polarisation) then
       deallocate (xc_potential_up, xc_potential_dn, density_wk_up, &
            density_wk_dn, STAT=stat)
       if (stat/=0) call cq_abort ('Error deallocating arrays for &
            &spin polarised calculation in get_pcc_force ', stat)
       call reg_dealloc_mem (area_moveatoms, 4*size, type_dbl)
    end if
    call stop_timer (tmr_std_allocation)
    return    
...
!! SOURCE
!!
  subroutine get_dxc_potential_LSDA_PW92 (density_up, density_dn, &
       dxc_potential_up_ddensity_up, dxc_potential_up_ddensity_dn, &
       dxc_potential_dn_ddensity_up, dxc_potential_dn_ddensity_dn, &
       size)
    ...
    ! passed variables
    integer :: size
    real(double), dimension(:) :: density_up(size), density_dn(size), &
         dxc_potential_up_ddensity_up(size), &
         dxc_potential_up_ddensity_dn(size), &
         dxc_potential_dn_ddensity_up(size), &
         dxc_potential_dn_ddensity_dn(size)
    
    ! local variables
    integer :: n
    real(double) :: rho_up, rho_dn, rho
    real(double) :: dVx_up_drho_up, dVx_up_drho_dn, dVx_dn_drho_up, &
         dVx_dn_drho_dn
    real(double) :: rs, rcp_rs, zeta, sq_rs, Q0, Q0p, Q1, Q1p, Q1pp, &
         e_c0, e_c1, malpha_c, f, de_c0_drs, de_c1_drs, dmalpha_c_drs,&
         df_dzeta, d2e_c0_drs2, d2e_c1_drs2, d2malpha_c_drs2, &
         d2f_dzeta2, drs_drho_up, drs_drho_dn, dzeta_drho_up, &
         dzeta_drho_dn, de_c_drs, de_c_dzeta, d2e_c_drs2, &
         d2e_c_drs_dzeta, d2e_c_dzeta2, dvc_up_drho_up, &
         dVc_up_drho_dn, dVc_dn_drho_up, dVc_dn_drho_dn, d2malpha_drs,&
         df2_dzeta2
    ...
    do n = 1, n_my_grid_points
       ...
       if (rho_up > very_small) then
          dVx_up_drho_up = k01 / ((rho_up)**two_thirds)
       else
          dVx_up_drho_up = zero
       end if
       dVx_up_drho_dn = zero
       dVx_dn_drho_up = zero
       if (rho_dn > very_small) then
          dVx_dn_drho_dn = k01 / ((rho_dn)**two_thirds)
       else
          dVx_dn_drho_dn = zero
       end if
       ...
       ! drs_drho_up, drs_drho_dn, dzeta_drho_up and dzeta_drho_dn
       if (rho > very_small) then
          drs_drho_up = - third * rs / rho
          drs_drho_dn = - third * rs / rho
          dzeta_drho_up = (one - zeta) / rho
          dzeta_drho_dn = - (one + zeta) / rho
       else
          drs_drho_up = zero
          drs_drho_dn = zero
          dzeta_drho_up = zero
       ...
       dVc_up_drho_up = (two_thirds * de_c_drs - one_third * rs * &
       ...
       dxc_potential_up_ddensity_up(n) = dVx_up_drho_up + &
            dVc_up_drho_up
       dxc_potential_up_ddensity_dn(n) = dVx_up_drho_dn + &
            dVc_up_drho_dn
       dxc_potential_dn_ddensity_up(n) = dVx_dn_drho_up + &
            dVc_dn_drho_up
       dxc_potential_dn_ddensity_dn(n) = dVx_dn_drho_dn + &
            dVc_dn_drho_dn
